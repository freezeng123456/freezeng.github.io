...
function generativeStackShift(lang) {
  const t =
    lang === "zh"
      ? {
          kicker: "生成式推荐",
          title: "三段链路，同一种改写",
          subtitle: "召回、粗精排与重排的短板都来自判别式形式本身，因此可以用同一种方式替换",
          lanes: [
            [
              "召回",
              "生成对象：视频的语义 ID",
              "索引决定了交叉能力的上限",
              "为了能建索引，交叉只能放在顶层内积",
              "近邻检索有折损，结果趋同，价值口径与后链路不同",
              "自回归生成 / 扩散还原",
              "直接生成 token 序列，不再需要近邻索引",
              "可做全负例训练，检索与排序共用同一套目标",
            ],
            [
              "粗精排",
              "生成对象：目标视频的多目标打分",
              "两阶段序列与手工交叉结构",
              "剪枝丢信息，只刻画目标与历史的相似度",
              "Dense 交叉靠启发式结构，参数放大后收益停滞",
              "端到端长序列 + 可放大的 Dense",
              "Cross-mask 直接建模多特征域的 3D 序列信号",
              "正交化更新解开梯度坍缩，Dense 得以继续 Scaling",
            ],
            [
              "重排",
              "生成对象：整条曝光序列",
              "多通道漏斗与规则化拼装",
              "数十条生成通道各自为战，规则决定最终拼装",
              "候选只有数十量级，评估退化成一个标量",
              "统一自回归生成 + 复杂评估",
              "一个模型生成整条序列，强化学习优化整体价值",
              "候选覆盖精排全量，奖励扩展到多窗口多行为",
            ],
          ],
          footer: "三处短板都不是调参问题：损失来自「先检索、再打分、再拼装」这一形式",
          chips: ["统一价值口径", "端到端信息通路", "用算力换建模能力"],
        }
      : {
          kicker: "GENERATIVE RECSYS",
          title: "Three Stages, One Rewrite",
          subtitle:
            "Retrieval, ranking, and re-ranking fail for the same reason: the discriminative form itself",
          lanes: [
            [
              "Retrieval",
              "generates: the item semantic ID",
              "The index caps interaction depth",
              "Crossing must sit on top, as an inner product",
              "Search is lossy, results converge, objectives diverge",
              "Autoregressive or diffusion generation",
              "Decode a token sequence with no nearest-neighbour index",
              "Full-negative training shares one objective with ranking",
            ],
            [
              "Ranking",
              "generates: multi-objective scores",
              "Two-stage sequences, handcrafted crossing",
              "Pruning drops information; only similarity survives",
              "Heuristic dense crossing stops paying as it grows",
              "End-to-end sequences, scalable dense layers",
              "Cross-mask attention models the 3D multi-field signal",
              "Orthogonalised updates unblock further dense scaling",
            ],
            [
              "Re-ranking",
              "generates: the whole exposure list",
              "A rule funnel fed by scattered channels",
              "Dozens of channels compete without coordination",
              "Dozens of candidates; evaluation is one scalar",
              "Unified generation with rich evaluation",
              "One model emits the list; RL optimises its total value",
              "Candidates cover the full ranking pool; rewards expand",
            ],
          ],
          footer: "None of the three is a tuning problem: retrieve, score, assemble is what loses it",
          chips: ["one shared objective", "end-to-end signal path", "compute buys capacity"],
        }
  const accents = [C.teal, C.indigo, C.amber]
  const fills = [C.tealSoft, C.indigoSoft, C.amberSoft]
  const lane = (i, row) => {
    const y = 180 + i * 185
    const accent = accents[i]
    return `${section(35, y, 1330, 170, row[0], accent, { subtitle: row[1] })}
    ${card(70, y + 58, 470, 100, { title: row[2], body: [row[3], row[4]], accent: C.rose, fill: C.roseSoft, titleSize: 16, bodySize: 12.5 })}
    ${lineArrow(556, y + 108, 626, y + 108, { color: accent, width: 3 })}
    ${card(646, y + 58, 694, 100, { title: row[5], body: [row[6], row[7]], accent, fill: fills[i], titleSize: 16, bodySize: 12.5 })}`
  }
  const chip = (i, label) =>
    pill(45 + i * 447, 772, 415, label, accents[i], fills[i], { h: 46, size: 14 })
  const body = `
    ${lane(0, t.lanes[0])}
    ${lane(1, t.lanes[1])}
    ${lane(2, t.lanes[2])}
    ${textBlock(700, 752, [t.footer], { size: 15, weight: 650, fill: C.muted })}
    ${chip(0, t.chips[0])}
    ${chip(1, t.chips[1])}
    ${chip(2, t.chips[2])}
  `
...
function generativeRerankUnification(lang) {
  const t =
    lang === "zh"
      ? {
          kicker: "生成式重排",
          title: "把漏斗改写成一次生成，再向两端 Scaling",
          subtitle: "统一模型要同时扛住生成质量、可调参性与检索效率，评估系统反过来提供训练奖励",
          funnelTitle: "常见形态：通道漏斗",
          funnel: ["生成通道 1", "生成通道 2", "…… 数十条通道", "规则化拼装", "候选：数十量级"],
          funnelNote: "评估退化为单一标量",
          modelTitle: "统一自回归生成模型",
          rlTitle: "强化学习：优化整条序列的价值",
          rlBody: ["评估打分落盘成奖励，先按排名归一化", "组内优势 + 梯度截断 + 序列级重要性采样"],
          insTitle: "指令跟随：把线上策略写进输入",
          insBody: ["规则压成四元组〈起点，窗口，下限，上限〉", "公式项连同超参一起输入，对改动泛化"],
          setTitle: "集合生成：生成单位从序列升到集合",
          setBody: ["每步把候选序列压成向量，彼此感知后再展开", "通道级奖励强化整体胜出的集合"],
          evalTitle: "可扩展的评估系统",
          axisA: "窗口：即时 · 会话 · 长期",
          axisB: "算子：求和 · 取最大 · 取最小",
          axisC: "行为：时长 · 互动 · 退出",
          axesNote: "三轴的乘积构成 List 级奖励",
          pointNote: "再叠加 Point 级信号",
          feedback: "推理阶段的序列打分落盘，成为训练阶段的奖励",
          forward: "向前：候选覆盖精排全量，精重排之间不再有漏斗",
          backward: "向后：评估复杂化，List 与 Point 信号同步扩展",
        }
      : {
          kicker: "GENERATIVE RE-RANKING",
          title: "Rewrite the Funnel as One Generation, Then Scale Both Ends",
          subtitle:
            "One model must carry generation quality, controllability, and search efficiency at once",
          funnelTitle: "Common shape: a channel funnel",
          funnel: [
            "Generation channel 1",
            "Generation channel 2",
            "… dozens of channels",
            "Rule-based assembly",
            "Candidates: dozens",
          ],
          funnelNote: "Evaluation collapses to one scalar",
          modelTitle: "Unified autoregressive generator",
          rlTitle: "RL: optimise the value of the whole list",
          rlBody: [
            "Served scores are logged and rank-normalised",
            "Group advantage, clipped gradients, sequence ratios",
          ],
          insTitle: "Instructions: policies enter the input",
          insBody: [
            "Rules compress to start, window, floor, ceiling",
            "Formula terms arrive with their hyper-parameters",
          ],
          setTitle: "Sets: the generated unit becomes a set",
          setBody: [
            "Each step compresses candidate lists into vectors",
            "Channel-level rewards favour the set that wins",
          ],
          evalTitle: "Scalable evaluation",
          axisA: "Windows: instant · session · long-term",
          axisB: "Operators: sum · max · min",
          axisC: "Behaviours: dwell · interaction · exit",
          axesNote: "Their product forms the list-level reward",
          pointNote: "Point-level signals stack on top",
          feedback: "Scores logged while serving become the rewards used in training",
          forward: "Forward: candidates cover the full ranking pool, so the funnel disappears",
          backward: "Backward: evaluation grows as list and point signals scale together",
        }
  const step = (i, label, color, fill) =>
    pill(58, 248 + i * 48, 244, label, color, fill, { h: 40, size: 13 })
  const axis = (i, label, color, fill) =>
    pill(1032, 250 + i * 56, 311, label, color, fill, { h: 44, size: 13 })
  let lattice = ""
  for (let r = 0; r < 3; r++) {
    for (let c = 0; c < 8; c++) {
      lattice += `<circle cx="${1050 + c * 36}" cy="${468 + r * 26}" r="5.5" fill="${[C.teal, C.blue, C.green][r]}" opacity=".5"/>`
    }
  }
  const body = `
    ${section(35, 180, 290, 425, t.funnelTitle, C.rose)}
    ${step(0, t.funnel[0], C.rose, C.roseSoft)}
    ${step(1, t.funnel[1], C.rose, C.roseSoft)}
    ${step(2, t.funnel[2], C.rose, C.roseSoft)}
    ${lineArrow(180, 394, 180, 416, { color: C.rose, width: 2.4 })}
    ${pill(58, 422, 244, t.funnel[3], C.amber, C.amberSoft, { h: 40, size: 13 })}
    ${lineArrow(180, 468, 180, 490, { color: C.amber, width: 2.4 })}
    ${pill(58, 496, 244, t.funnel[4], C.rose, C.white, { h: 40, size: 13 })}
    ${textBlock(180, 566, [t.funnelNote], { size: 12.5, weight: 600, fill: C.muted })}

    ${section(375, 180, 590, 425, t.modelTitle, C.indigo)}
    ${card(400, 248, 540, 100, { title: t.rlTitle, body: t.rlBody, accent: C.blue, fill: C.blueSoft, titleSize: 16, bodySize: 12.5 })}
    ${card(400, 362, 540, 100, { title: t.insTitle, body: t.insBody, accent: C.teal, fill: C.tealSoft, titleSize: 16, bodySize: 12.5 })}
    ${card(400, 476, 540, 100, { title: t.setTitle, body: t.setBody, accent: C.green, fill: C.greenSoft, titleSize: 16, bodySize: 12.5 })}

    ${section(1010, 180, 355, 425, t.evalTitle, C.teal)}
    ${axis(0, t.axisA, C.teal, C.tealSoft)}
    ${axis(1, t.axisB, C.blue, C.blueSoft)}
    ${axis(2, t.axisC, C.green, C.greenSoft)}
    ${textBlock(1187, 436, [t.axesNote], { size: 12.5, weight: 650, fill: C.muted })}
    ${lattice}
    ${pill(1032, 548, 311, t.pointNote, C.amber, C.amberSoft, { h: 40, size: 13 })}

    ${lineArrow(332, 392, 368, 392, { color: C.rose, width: 3 })}
    ${lineArrow(972, 392, 1004, 392, { color: C.indigo, width: 3 })}
    ${pathArrow("M 1187 613 C 1187 674, 780 694, 672 618", { color: C.teal, width: 2.6, dashed: true })}
    ${textBlock(930, 712, [t.feedback], { size: 13.5, weight: 650, fill: C.teal })}
    ${pill(45, 740, 640, t.forward, C.amber, C.amberSoft, { h: 46, size: 14 })}
    ${pill(715, 740, 640, t.backward, C.green, C.greenSoft, { h: 46, size: 14 })}
  `
  return frame({
    width: 1400,
    height: 810,
...
function crossMaskFieldSignal(lang) {
  const t =
    lang === "zh"
      ? {
          kicker: "长序列建模",
          title: "推荐的序列信号比文本多一维",
          subtitle: "文本只有「序列 × 表示」，推荐还有「特征域」，cross-mask 正是为这一维设计的",
          textTitle: "文本任务：单域序列",
          textDim: "一个特征域，两个维度",
          textNote: "注意力只需刻画 token 与 token 的关联",
          recTitle: "推荐任务：多域序列",
          recDim: "序列 × 特征域 × 表示，三个维度",
          historyLabel: "历史行为",
          targetLabel: "目标",
          domainA: ["内容域", "视频 · 作者 · 类目"],
          domainB: ["行为域", "完播 · 点赞 · 转发"],
          domainC: ["上下文域", "位置 · 时刻 · 场景"],
          crossTitle: "Cross-mask 注意力",
          stepA: ["分域计算注意力", "每个特征域单独得到一份 score", "对应不同视角下的行为间关联"],
          stepB: ["分数乘以域掩码", "某一域的 score 与另一域的掩码相乘", "域间交叉因此成为显式结构"],
          stepC: ["跨域聚合取值", "用交叉后的分数聚合各域的 value", "得到特征域的二阶交叉表示"],
          costTag: "成本控制",
          costA: "计算共享",
          costB: "零训练外推",
          costC: "零显存计算",
          costNote: "自注意力开销下降一半以上",
        }
      : {
          kicker: "LONG-SEQUENCE MODELLING",
          title: "A Recommendation Sequence Carries One Extra Axis",
          subtitle:
            "Text has sequence and representation; recommendation adds feature fields, and cross-mask targets that axis",
          textTitle: "Text: a single-field sequence",
          textDim: "one field, two dimensions",
          textNote: "Attention only relates one token to another",
          recTitle: "Recommendation: a multi-field sequence",
          recDim: "sequence × field × representation",
          historyLabel: "history",
          targetLabel: "target",
          domainA: ["Content", "video · author · category"],
          domainB: ["Behaviour", "finish · like · share"],
          domainC: ["Context", "slot · hour · scene"],
          crossTitle: "Cross-mask attention",
          stepA: [
            "Score each field alone",
            "Every field gets its own attention map",
            "Each map is one view of item-to-item affinity",
          ],
          stepB: [
            "Multiply scores by field masks",
            "One field's scores meet another field's mask",
            "Cross-field interaction becomes explicit",
          ],
          stepC: [
            "Aggregate across fields",
            "Crossed scores pool the values of every field",
            "The result is a second-order field interaction",
          ],
          costTag: "COST CONTROL",
          costA: "shared computation",
          costB: "training-free extrapolation",
          costC: "no extra activation memory",
          costNote: "Self-attention cost falls by more than half",
        }
  let tokens = ""
  for (let i = 0; i < 7; i++) {
    tokens += `<rect x="${58 + i * 52}" y="252" width="42" height="46" rx="12" fill="${C.blueSoft}" stroke="${C.blue}" stroke-opacity=".32"/>`
  }
  const rowColors = [C.amber, C.blue, C.green]
  const rowFills = [C.amberSoft, C.blueSoft, C.greenSoft]
  const domains = [t.domainA, t.domainB, t.domainC]
  let grid = ""
  for (let r = 0; r < 3; r++) {
    const y = 254 + r * 50
    grid += `${textBlock(487, y + 15, [domains[r][0]], { size: 14, weight: 750, fill: rowColors[r], anchor: "start" })}
      ${textBlock(487, y + 33, [domains[r][1]], { size: 11, weight: 550, fill: C.muted, anchor: "start" })}
      <rect x="1288" y="${y}" width="62" height="42" rx="10" fill="${C.white}" stroke="${C.rose}" stroke-opacity=".55" stroke-dasharray="5 5"/>`
    for (let c = 0; c < 7; c++) {
      grid += `<rect x="${650 + c * 88}" y="${y}" width="80" height="42" rx="10" fill="${rowFills[r]}" stroke="${rowColors[r]}" stroke-opacity=".34"/>`
    }
  }
  const stepCard = (i, entry, accent, fill) =>
    card(58 + i * 442, 518, 400, 118, {
      title: entry[0],
  ...
  const body = `
    ${section(35, 180, 400, 250, t.textTitle, C.blue)}
    ${tokens}
    ${textBlock(235, 336, [t.textDim], { size: 13, weight: 700, fill: C.blue })}
    ${pill(58, 360, 354, t.textNote, C.blue, C.blueSoft, { h: 44, size: 12.5 })}

    ${section(465, 180, 900, 250, t.recTitle, C.amber)}
    ${textBlock(954, 244, [t.historyLabel], { size: 12, weight: 650, fill: C.muted })}
    ${textBlock(1319, 244, [t.targetLabel], { size: 12, weight: 650, fill: C.rose })}
    ${grid}
    ${textBlock(915, 418, [t.recDim], { size: 13, weight: 700, fill: C.amber })}
    ${lineArrow(915, 436, 915, 450, { color: C.amber, width: 2.6 })}

    ${section(35, 452, 1330, 210, t.crossTitle, C.indigo, { subtitle: "σ(QKᵀ) ⊙ M" })}
    ${stepCard(0, t.stepA, C.teal, C.tealSoft)}
    ${stepCard(1, t.stepB, C.blue, C.blueSoft)}
    ${stepCard(2, t.stepC, C.amber, C.amberSoft)}
    ${lineArrow(466, 577, 492, 577, { color: C.indigo, width: 2.6 })}
    ${lineArrow(908, 577, 934, 577, { color: C.indigo, width: 2.6 })}

    ${textBlock(54, 692, [t.costTag], { size: 13, weight: 800, fill: C.muted, anchor: "start" })}
    <path d="M 55 704 L 1345 704" stroke="${C.line}" stroke-width="1.5" stroke-dasharray="5 8"/>
    ${pill(45, 720, 400, t.costA, C.teal, C.tealSoft, { h: 42, size: 13 })}
    ${pill(475, 720, 420, t.costB, C.indigo, C.indigoSoft, { h: 42, size: 13 })}
    ${pill(925, 720, 430, t.costC, C.green, C.greenSoft, { h: 42, size: 13 })}
    ${textBlock(700, 792, [t.costNote], { size: 14, weight: 700, fill: C.ink })}
  `
  return frame({
    width: 1400,
    height: 830,
...
const diagrams = [
  ...
  ["generative-recommendation", "stack-shift", generativeStackShift],
  ["generative-recommendation", "rerank-unification", generativeRerankUnification],
  ["generative-recommendation", "cross-mask-field-signal", crossMaskFieldSignal],
