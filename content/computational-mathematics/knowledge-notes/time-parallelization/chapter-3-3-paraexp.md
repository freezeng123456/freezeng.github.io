...
## 3.4.5 Theorem 3.4：有限步收敛与 Parareal 等价
...
## 3.4.6 Figure 3.8：Burgers 方程上的拆分失效
...
横线表示离散截断误差 $\max\{\Delta t,\Delta x^2\}$，实际迭代达到这条线后即可停止。强扩散时，$A$ 捕获主要动力学，ParaExp 收敛很快并优于标准 Parareal。黏性下降后，$B(\boldsymbol u)$ 主导输运，标准 Parareal 一度更快；$\nu=0.02$ 时 ParaExp 已发散。继续减小黏性，标准 Parareal 最终也会失效。
...
| 原文项目                     | 本页位置    | 覆盖状态                                           |
| ---------------------------- | ----------- | -------------------------------------------------- |
| (3.13)–(3.16), Figure 3.7    | 3.4.1–3.4.3 | 两组子问题、精确叠加、归纳证明、指数作用与完整原图 |
| (3.17)–(3.19)                | 3.4.4       | 非线性拆分、冗余来源和改写后的迭代                 |
| (3.20a)–(3.20c), Theorem 3.4 | 3.4.5       | 有限步结论、归纳理由及 Parareal 传播子             |
| (3.21), Figure 3.8           | 3.4.6       | Burgers 离散、全部时间参数、截断误差线与三阶段结论 |
