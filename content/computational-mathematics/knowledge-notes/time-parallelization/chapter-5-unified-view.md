...
论文按“解会不会遗忘信息”区分两类问题。抛物方程在演化中遗忘大量信息，解因而具有较强的时间局部性。很多方法可以利用这项性质，包括 Parareal、STMG、ParaExp、ParaDiag，以及以区域分解为基础的波形松弛。

双曲方程会在很长时间内保留非常精细的解结构，可用方法的范围更窄。论文重点列出 ParaExp、ParaDiag 和 SWR，尤其强调 SWR 与 tent pitching 的联系。

作者推荐 Gander 与 Lunet（2024, SIAM）的专著作为进一步阅读。该书为各类 PinT 方法提供历史背景、自包含的收敛分析和可直接运行的 MATLAB 短程序。论文全部结果所用的代码公开在 [wushulin/ActaPinT](https://github.com/wushulin/ActaPinT)。

> [!note] 本站补充：从结论到选型
> 下面这条初筛原则是本站对上述结论的引申，不是原文的表述：先判断问题会不会快速遗忘高频信息，再选择时间并行结构；算法名称或传统分类本身无法替代这项动力学判断。本页后续用“时间记忆”概括这一区别，该词同样是本站的说法。原文的措辞是抛物问题“在时间上遗忘大量信息、解在时间上是局部的”，双曲问题“在很长时间内保留非常精细的解结构”。
