...
> [!note] 内容边界
> 第 5.1 节对应论文 Section 5（p. 481）。第 5.2 节以后是本站对第二至第四章的统一整理与复现记录，包含作者原文没有展开的代数比较、Python 实验、T4 GPU 性能和报告规范。

## 5.1 论文的最终结论

论文用“时间记忆”概括双曲与抛物问题对 PinT 的不同要求。抛物方程会在演化中遗忘大量精细信息，解因而具有较强的时间局部性。很多方法可以利用这项性质，包括 Parareal、STMG、ParaExp、ParaDiag，以及以区域分解为基础的波形松弛。

双曲方程会在很长时间内保留精细结构、相位和传播路径，可用方法的范围更窄。论文重点列出 ParaExp、ParaDiag 和 SWR，尤其强调 SWR 与 tent pitching 的联系。这些方法以特征传播、矩阵指数或全时间代数结构组织并行，减少了对耗散型粗时间模型的依赖。

作者推荐 Gander 与 Lunet（2024）的专著作为进一步阅读。该书为各类 PinT 方法提供历史背景、自包含的收敛分析和可直接运行的 MATLAB 短程序。论文 Figures 2–4 使用的代码公开在 [wushulin/ActaPinT](https://github.com/wushulin/ActaPinT)。

这段结论给出一条实用的初筛原则：先判断问题会不会快速遗忘高频信息，再选择时间并行结构。算法名称或传统分类本身无法替代这项动力学判断。

## 5.2 本站统一解释：求解同一个全时间系统
...
公式 (5.1) 给出三个共同问题：$M^{-1}$ 能削弱哪些误差模态？它是否保留方程中的相位、平均值和激波位置？应用一次 $M^{-1}$ 时，哪些计算真正并发，哪些部分仍然顺序执行？
...
| Section 3.5，pp. 415–443      | 第三章 3.5–3.8 | 覆盖 ParaDiag-I/II、Theorems 3.5–3.9、BVM、NKA、循环与 $\alpha$-循环实验      |
| Sections 4.1–4.4，pp. 443–460 | 第四章 4.1–4.4 | 覆盖 Parareal、PFASST、MGRiT、Theorems 4.1–4.6 与 Figures 4.1–4.11            |
| Sections 4.5–4.6，pp. 460–481 | 第四章 4.5–4.7 | 覆盖两种对角化 Parareal、STMG、Theorems 4.7–4.9、Figures 4.12–4.22、Table 4.1 |
| Section 5，p. 481             | 本章 5.1       | 覆盖双曲/抛物总结、推荐方法、专著与公开代码                                   |
