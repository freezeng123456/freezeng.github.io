...
description: Parareal、PFASST、MGRiT、对角化 Parareal 与时空多重网格的完整分析，含 Section 4 导论的设计动机与 4.1 历史发展脉络
lang: zh
...
第二章已经从直觉上说明：抛物问题的解在时间上是**局部的**——除了极少数很低频的分量，信息会被扩散迅速抹平；而双曲问题的解在时间上高度**非局部**，并且这种非局部性覆盖从最低到最高的所有频率分量。第三章的方法正是为双曲问题设计的，它们以非局部的方式同时处理所有频率。把这些方法用到抛物问题上时往往表现更好，因为在长时间窗内真正需要非局部处理的，只剩下那几个极低频、在时间上高度非局部的分量。

问题在于，第三章的方法大多是**为线性问题设计**、并在线性情形下最有效；面对非线性时各自都有短板：

- SWR（尤其是 optimized SWR）需要确定传输/Robin 参数，缺乏合理参数时收敛会很差；
- ParaExp 主要适用于线性问题；
- ParaDiag I、II 的非线性版本以 Newton 迭代作为外层求解器，非线性会拖慢 Newton 的收敛，时间窗较长时甚至可能发散。

本章转而讨论**专门为抛物问题设计**、直接利用其时间局部性的方法：Parareal、PFASST、MGRiT、两种对角化 Parareal 和时空多重网格（STMG）。它们对线性与非线性问题同样有效，收敛机制与第三章方法**完全不同**；也正因如此，把它们直接搬到双曲问题上通常收敛很慢，甚至发散。

### 耗散为什么让廉价粗传播成立

这些方法共享同一个前提：耗散。扩散以正比于频率平方的速率迅速衰减高频误差，于是较晚的状态对很久以前的精细信息不再敏感——高模态无论粗、细传播如何失配，都会被自身的强耗散压平，误差不会沿时间累积。这样一来，一个只需正确携带**慢模态**的廉价粗传播子就足以承担长程因果信息的传递；细传播只要在各大区间内并行补齐局部细节即可。

Parareal 的逐模态收敛因子 $\rho_\ell=\dfrac{|R_g-R_f^J|}{1-|R_g|}$ 精确刻画了这一机制：分子度量粗、细传播的差异，分母 $1-|R_g|$ 是粗传播的**耗散裕量**。抛物模态位于负实谱，$|R_g|$ 明显小于 $1$，分母把差异牢牢压住；一旦进入双曲极限、模态趋向虚轴，$|R_g|\to1$，分母塌缩，同样大小的粗细相位失配被急剧放大，因子越过 $1$。这也解释了为什么同一套粗网格校正机制在双曲问题上会失效：没有耗散，所有频率的相位与传播误差都不会自动消失，而能够准确表示这些误差的粗传播往往贵到失去成本优势。

前三类方法（Parareal、PFASST、MGRiT）主要在**时间**上构造粗细层级，通常保持同一个空间网格；STMG 则同时粗化空间与时间，并使用可沿时间并行的块 Jacobi 平滑器。

![抛物型时间并行方法的粗细层级与并行位置](assets/diagrams/pint/zh/parabolic-multilevel-map.svg)

> [!tip] 本站洞见
> 可以把 $1-|R_g|$ 读作"记忆遗忘速率"。抛物问题遗忘得快，廉价粗模型只需记住少数慢模态；双曲问题几乎不遗忘，任何被粗传播丢弃的相位信息都会长期留存并反噬迭代。这条主线统一解释了本章全部四种方法在小黏性、近双曲极限下的退化，也预告了第四章各图中 $\nu$ 下降时收敛因子一致向 $1$ 逼近的现象（Gander、Wu 与 Zhou, 2025）。

### 4.1 历史发展：从多重打靶到时空多重网格

**Parareal 一线。** 本章首先出场的是 Lions、Maday 与 Turinici (2001) 的 Parareal（第 3.4 节讨论非线性 ParaExp 变体时已提及）。它虽是在虚拟控制背景下独立提出的，根源却可追溯到更早的演化问题**多重打靶**技术（Bellen 与 Zennaro, 1989；Chartier 与 Philippe, 1993）。Saha、Stadel 与 Tremaine (1997) 在太阳系模拟中已经给出用"粗模型"（而非粗网格）代替昂贵传播的同一算法，并指出它与波形松弛的联系。更早的先驱是 Nievergelt (1964)，不过那里的方法**还不是迭代的**——它一次性预计算若干候选轨迹再拼接。Parareal 提出二十余年，收敛性已被充分理解（Gander 与 Vandewalle, 2007；Gander 与 Hairer, 2008, 2014；Gander 与 Lunet, 2024），并成为衍生更高效 PinT 方法的模板。

**从 Parareal 生长出的变体。** 文献中有大量改造，用以适配不同问题或目的：并行隐式时间积分算法 PITA（Farhat 与 Chandesris, 2003；Farhat 等, 2006；Cortial 与 Farhat, 2009）、时空并行全近似格式 PFASST（Minion, 2011；Emmett 与 Minion, 2012；Minion 等, 2015）、时间多重网格缩减 MGRiT（Falgout 等, 2014；Dobrev 等, 2017；Hessenthaler 等, 2020），以及 Parareal 与 ParaDiag 的组合（Wu, 2018；Gander 与 Wu, 2020）。基于 Parareal 的 PinT 方法有一个共同特征：在**时间**离散上使用两层（或更多）网格，而**空间**离散只用一层网格。

**时空多重网格一线。** 在空间和时间上同时使用多重网格的思路可上溯至 Hackbusch (1984) 的抛物多重网格，Lubich 与 Ostermann (1987) 以多重网格波形松弛的形式给出了优雅的分析。这一路线中，时间方向的粗化长期难以奏效；针对强对流问题的重要改进见 Vandewalle 与 Van de Velde (1994)、Horton 与 Vandewalle (1995)、Janssen 与 Vandewalle (1996) 以及 Van Lent 与 Vandewalle (2002)。真正打通时间粗化的是 Gander 与 Neumüller (2016)：他们只用标准多重网格部件，但把沿时间的**块 Jacobi 平滑器**作为新的关键成分引入并分析，这就是 STMG。它是目前抛物问题上最有力的 PinT 算法之一，具有出色的强、弱扩展性（另见 Neumüller 与 Smears, 2019），本章在最后介绍它，并展示它对非线性抛物问题同样有效。

> [!tip] 本站洞见
> 两条历史主线的分野正是"侵入性"：Parareal → PITA → PFASST/MGRiT 以两层时间网格封装既有的粗、细传播子，尽量保持非侵入；Hackbusch → Gander 与 Neumüller (2016) 的时空多重网格则要求访问全时间离散、平滑器与网格传递，侵入性更高，换来的是理论上最彻底、可扩展性最强的时间粗化。理解这一取舍，就能预判在既有代码库上应优先尝试哪一族方法。
...
> [!note] 关于 PFASST 的两种等价表述
> 本页沿用 FAS/$\tau$ 校正的表述，这是全近似格式的**通用命名方式**。论文 §4.3 给出的**精确刻画**则是块迭代形式（Gander 等, 2023b）：两层 PFASST 的更新由三个块算子 $B_{01}$、$B_{10}$、$B_{00}$ 组成，示意地写作
>
> $$
...
> $$
>
> 其中各块由细、粗配置矩阵、其隐式 Euler 近似 $\tilde M$ 以及节点间的传递矩阵 $\mathcal T_{f\to c}$、$\mathcal T_{c\to f}$ 组合而成（例如 $B_{10}=\mathcal T_{c\to f}\tilde M_c^{-1}\mathcal T_{f\to c}$ 一类结构）。两种表述指向同一算法：块迭代形式更便于收敛分析，FAS/$\tau$ 则说明其多重网格来历。
...
论文讨论两种不同的对角化位置。第一种把 $N$ 个大区间上的顺序粗网格校正改写为可 FFT 对角化的 head-tail 系统；第二种在每个大区间内部，用 $J$ 个细时间点上的 $\alpha$-循环系统构造低成本粗传播。

### 4.5.1 基于对角化的 CGC

标准 Parareal 的粗校正按时间顺序执行。论文加入条件
...
直接把 head-tail 条件放进 MGRiT 会改变其不动点并导致发散。论文给出一致的替代条件，让首尾耦合使用最近两轮迭代之差。小 $\alpha$ 时，它与原 MGRiT 保持相同收敛率；这个条件也可以用于 Parareal。
...
时间高频的平滑因子至多为 $1/\sqrt2$，因此可以安全地做时间粗化。在论文归一化的热方程中，若 $\Delta t/\Delta x^2\ge1/\sqrt2$，空间高频也被压到同一上界，可以同时做空间粗化；恢复扩散系数后，对应的无量纲比为 $\nu\Delta t/\Delta x^2$。ADE 的符号含有虚部，论文据此仍建议把 $\eta=1/2$ 作为后向 Euler 的合理起点。
...
强扩展中，一个含 512 个时间步、15,300,608 个自由度的问题从 1 核的 7,635.2 秒降到 256 核的 30.0 秒；更大的 524,288 时间步问题从 512 核的 15,205.9 秒降到 262,144 核的 30.0 秒。这些数据来自论文引用的三维实现，展示了 STMG 在抛物问题上的弱、强扩展潜力。
...
综合 Section 4.6，STMG 是当前抛物问题上最有力的 PinT 方法之一，并已展示大规模扩展。它需要访问全时间离散、平滑器和网格传递，侵入性高于 Parareal。对双曲问题和不同时间积分器的鲁棒性仍需进一步研究。
...
ADE 的离散采样最小点为 $0.372$，而论文理论与图示把 $1/2$ 作为稳健选择。两者衡量的是特定有限网格上的 15-cycle 误差和高频平滑上界，结论层次不同。
...
| 原文位置                       | 本页对应 | 已覆盖内容                                                                                  |
| ------------------------------ | -------- | ------------------------------------------------------------------------------------------- |
| Sections 4 与 4.1，pp. 443–444 | 4.1      | 抛物时间局部性、第三章方法的非线性局限、耗散支撑粗校正的机制、Parareal 与 STMG 两条历史主线 |
| Section 4.2，pp. 444–452       | 4.2      | Parareal 更新、Theorems 4.1–4.4、Figures 4.1–4.5、非线性与双曲退化                          |
| Section 4.3，pp. 452–455       | 4.3      | 配置方程、SDC、FAS 传递、块迭代（B01/B10/B00）刻画、PFASST 迭代与 Figure 4.6                |
