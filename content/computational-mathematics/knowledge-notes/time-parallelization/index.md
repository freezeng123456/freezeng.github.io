...
> [!note] 数值来源
> 页面中的原论文图表直接取自论文 PDF，并保留原坐标与面板。标为“本站复现”的实验于 2026 年 7 月 31 日在新的实验服务器上由 Python 项目重新生成。初始正式结果使用 SciPy CPU 路径；随后加入 CuPy/T4 混合后端，将独立 Burgers 细传播批量放到 GPU。Figure 4.5 的停止迭代保持一致，完整论文套件由 263.57 秒降至 67.92 秒。
