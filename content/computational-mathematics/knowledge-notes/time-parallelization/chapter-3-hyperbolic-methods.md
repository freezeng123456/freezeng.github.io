...
Figure 3.2 取 $L=8.2$、$T=5$、$\Delta t=0.01$、$\Delta x=0.02$、$l=2\Delta x$ 和 Gaussian 初值。数值曲线验证了黏性下降时收敛改善，也显示 Robin 条件明显优于 Dirichlet。四子域实验中，$\nu=0.1$ 时 Dirichlet 与优化 Robin 分别约需 92 和 28 次迭代；二子域连续理论预测约为 32 和 4 次。差距来自连续/离散、二子域/四子域以及边界设置的不同，不能把理论界直接当作该离散实验的精确迭代数。
...
Figure 3.3 用特征锥展示这一过程。每个子域中已有一部分解与精确解一致，下一轮通过界面数据把这片正确区域继续扩大。这个几何解释也引出红黑 SWR：相邻时空块按颜色并行计算，允许一定冗余工作，以换取更大的并发度。
...
IDC 先用一个低阶方法得到预测解，再离散求解这个误差方程并更新 $u^{k+1}=u^k+e^k$。论文给出 $\theta$ 方法的离散校正式和由插值积分产生的权重，最终形成节点递推 (3.11)。校正使用积分残差，避免直接数值微分一个噪声残差。
...
论文把线性齐次部分继续交给指数传播，把 $B(\boldsymbol u)$ 放入迭代的非齐次局部问题。Theorem 3.4 给出两个重要结论：第 $k$ 次迭代后，前 $k$ 个时间子区间已经与串行细解一致；在粗时间节点上，这个迭代等价于一个简化 Parareal，其中粗传播子是线性齐次演化，细传播子处理完整非线性。
...
近似 Jacobian 还可扩展为低秩 Kronecker 和
