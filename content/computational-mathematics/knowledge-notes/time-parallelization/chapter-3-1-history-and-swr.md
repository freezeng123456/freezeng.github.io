...
论文在进入具体算法前先给出三条判断。第一，为双曲问题设计的 PinT 方法用于抛物问题时通常同样有效，甚至更好。第二，mapped tent pitching 是这条规律的例外，因为它建立在有限传播速度之上，而抛物问题的传播速度无限。第三，反方向不成立：第四章那些为抛物问题设计的方法，用于双曲问题时一般都会失效。
...
> [!warning] 原文公式核对：$p^*$ 中的符号 $s$
> 期刊版印出的缩放是 $p^*=\widetilde p^*s/\nu$，但 $s$ 在全文其他位置都没有定义；按模型问题 (2.5) 的单位对流速度读，它退化为 $p^*=\widetilde p^*/\nu$。arXiv 预印本在同一处印的是倒数形式 $p^*=\widetilde p^*\nu/s$。本页按期刊版排印，并在此标注该出入，避免读者对照两个版本时产生困惑。
...
在 $\nu=0.1$ 时，Dirichlet 与优化 Robin 实测分别需要 92 和 28 轮；二子域连续理论预测 32 和 4 轮。差异来自三项设置变化：理论使用无界域，实验使用有界域；理论只有两个子域，实验有四个；实验还离散了空间和时间。论文据此提醒，Figure 3.1(a) 描述趋势和理想因子，不能直接充当多子域离散实现的迭代数公式。

理论覆盖情况并不均匀。Dirichlet 传输条件的多子域收敛分析见 Gander 与 Stuart（1998）以及 Wu、Huang 与 Huang（2012）；两子域半离散 Robin 情形见 Wu 与 Al-Khaleel（2014），稳态情形见 Gander、Halpern、Hubert 与 Krell（2020, 2021a）。但 Robin 传输条件的多子域完整收敛理论仍然缺失，这是论文明确指出的空白。

### 更精确的传输条件

Ventcel 条件（Bennequin et al. 2016）用局部高阶算子逼近最优的非局部传输。论文给出的 Fourier 空间最优算子（Gander 与 Halpern 2007, Section 3）具有形式
...
论文接着指出这条性质的关键推论：既然每个子域在只受初值影响的锥形区域内已经得到精确解，就可以据此选择时空子域，**完全避免迭代**，直接用并行的时空子域求解沿时间推进。这一观察最早出现在 Gander 等（2003）。下面的 tent pitching 正是这条路线的实现。
...
Mapped tent pitching（MTP，Gopalakrishnan、Schöberl 与 Wintersteiger 2017；时域 Maxwell 应用见 Gopalakrishnan、Hochsteger、Schöberl 与 Wintersteiger 2020）把倾斜 tent 映射成时空柱体，再用经典时间步进求解，最后映回原几何。它省去红黑 SWR 中的冗余区域，但要额外付出计算映射的成本；论文明确指出，映射之后的计算区域与红黑 SWR 的时空子域大小相同，因此**总计算量相当**，MTP 并不因为消除冗余而更便宜。映射还可能导致阶数下降，需要专门设计积分器。
