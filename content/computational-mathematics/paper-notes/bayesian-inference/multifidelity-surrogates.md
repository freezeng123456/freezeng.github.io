...
写作前先确定每篇的可核实程度，后文的详略完全按这张表分配：**只有全文核对的论文才给出完整推导与实验配置**，其余部分只写能落到源头的内容。

| 编号 | 核对来源                           | 层级         | 残留缺口                                                                    |
| ---- | ---------------------------------- | ------------ | --------------------------------------------------------------------------- |
| 37   | arXiv:1807.00618（ar5iv 全文）     | **全文核对** | 论文未给出可转录的误差数值表，本页数值实验一节只能报告配置与摘要级结论      |
| 34   | arXiv:1809.08931（全文）+ 期刊摘要 | **全文核对** | 总代价记法 $J_1+Q_1+J_2Q_2$ 中 $J_1,J_2$ 的定义在提取中断裂，未核实         |
| 49   | arXiv:1911.08926（全文）           | **全文核对** | 式 (10) 的接受概率分子分母疑似互换，见下文警示框；自适应版本无 CPU 时间数值 |
| 79   | arXiv:2310.17844 v3（全文）        | **全文核对** | 无实质缺口；定理 3.6 的适用范围受限于线性算子网络，见下文警示框             |

四篇都属于全文核对层级，因此本页对四篇一律给出推导链、定理的完整假设与可复现的实验配置。缺的是**测量值本身**：只有编号 34 在可核实材料中留下了完整的误差与代价数字，编号 37、49、79 留下的是配置和定性结论。这一点在每篇的数值实验一节里都单独说明。
...
还剩一个问题：链是用代理跑的，它走到的地方是**代理的后验**主体，不一定是真后验主体。如果完全信任代理的接受概率，链就会在代理自己的高概率区里打转，指标在那里当然合格，细化永远不被触发，误差被自己确认成了没有误差。论文的补丁是在每一轮外循环里插入一步用**真模型接受概率**的 Metropolis 判定，用它产生的被接受点作为检验点。这一步的成本是每轮两次真求解（在 $z^-$ 和 $z^+$ 上各一次），换来的是检验点由真后验而不是代理后验挑选。**这是「代理放进 MCMC 需要接受步纠正」这句话在本族里的确切含义**——注意它纠正的是细化点的位置，不是样本本身，后一点见下文的诚实说明。
...
$w_i$ 正是离散形式的 Christoffel 函数，与[[computational-mathematics/paper-notes/stochastic-approximation/optimal-sampling-and-preconditioning|最优采样与预条件]]一页研究的是同一个对象：那里的稳定性因子是 $\|K\|_\infty/N$，即本页权的倒数取上确界，而诱导采样 $q\propto\sum_n v_n^{2}$ 就是让这个上确界降到它的下界。采样密度按度数渐近设计：均匀测度用张量积 Chebyshev，高斯测度用支撑在半径 $\sqrt{2N}$ 球上的密度（Narcowich–Ward 型设计）。
...
> [!warning] 这个算法采的是谁的后验
> 上一句值得停一下。子链是用当前低精度模型跑的，因此并集里的样本服从的是**代理诱导的后验** $\widetilde\pi^{d}_{N}$，不是真后验 $\pi^{d}$。真模型只出现在两个位置：挑选检验点，以及细化时的 $Q$ 次求值。所以这套算法的正确性依据不是 Metropolis 修正把样本纠回真后验（那是 Conrad–Marzouk–Pillai–Smith 的渐近精确性路线），而是下面的 Theorem 2：只要 $\widetilde\pi^{d}_{N}$ 与 $\pi^{d}$ 在 KL 意义下足够近，用前者代替后者就够了。两条路线的保证强度不同，转述时不要混淆。
...
界的形状本身说明了它的来源：两项对应把 KL 积分按 $\Gamma_{N}(\epsilon)$ 与其补集拆开——在可行集上势函数之差被 $\epsilon$ 控制，给出 $K_1\epsilon$；在补集上代理可以任意坏，只能用 Assumption 1 的一致界压住被积函数，再乘以那块区域的后验质量，给出 $K_2\mu(\Gamma^{\perp}_{N}(\epsilon))$。这解释了为什么需要一个看上去多余的一致有界假设。（这是本页对界的结构解读，论文本身只给出结论。）

> [!warning] 已证明的与只是论证的
> 论文对算法与定理之间的联系是明确表述为**机制**而非定理的：只要候选点落在 $\Gamma^{\perp}_{N}(\epsilon)$ 内，算法就在其邻域细化，因此 $\mu(\Gamma^{\perp}_{N}(\epsilon))$ 随细化渐近衰减。**这是一个论证，不是收敛速率，也没有给出 $\mu(\Gamma^{\perp}_{N}(\epsilon))$ 何时或以多快速度趋于零。** 本专题所有论文在这一点上的处境相同。
...
两个例子的分工是刻意的：例 1 排除了「自适应方法只是因为对手太弱才赢」这一解释，因为在 $n_z=2$ 上对手可以做到全局精确；例 2 才是方法的目标场景。摘要给出的量级判断是相对于纯真模型 MCMC 有若干数量级的效率提升。

> [!note] 这一篇的数值证据止步于此
> 可核实材料只包含上表的配置与摘要的量级表述，**没有可转录的误差数值、链长、CPU 时间或真求解次数**。因此本页不为编号 37 列结果表。想看这套方法带数字的对照，应看下一节的编号 34——它是同一个修正装进另一个采样器，且完整报告了误差与求解次数。
...
（这一步归约是标准的，本页照标准形式给出；Iglesias 等人的原文写的是分块前的形式。）编号 34 采用的正是这个式子，只在 $\Gamma$ 前多一个正则化参数 $\alpha_n$：
...
两条都来自 Iglesias 的正则化集合 Kalman 平滑器。**Iglesias 等人自己明确说明：完整的收敛与正则化分析超出该文范围，偏差原理是数值上支持的，不是被证明的。** 因此这条停机准则在本族里始终只是经验规则。同一来源还记录了一个反直觉的经验事实：把扰动数据换成不扰动（$\eta^{(j)}_{n+1}=0$）结果反而**更差**，作者猜测噪声帮助算法探索 $\mathcal A$。
...
**本篇不给出自适应格式的收敛定理，全部论断都是计算性的。** 论文正文出现过 $J_1+Q_1+J_2Q_2$ 形式的总代价记法，但 $J_1$ 与 $J_2$ 的定义在可核实材料中断裂，本页不复述。上一节引用的线性情形 Tikhonov–Phillips 极限与偏差原理都来自 Iglesias 等人，不是本篇的结果。
...
- 自适应版本的相对误差在可核实材料中**没有数值**，只有定性表述：局部修正基本恢复了全模型集合 Kalman 反演的精度。因此上表把这两格标为未报告，而不是填入 $0.0461$。
...
差的地方有四处。第一，自适应版本的 CPU 时间没有报告，因此「便宜」这个结论建立在真求解次数上，而不是实测墙钟时间上——多精度重拟合本身的开销没有被计入对照。第二，自适应版本的相对误差没有数值，只有定性判断，所以「基本恢复全模型精度」无法量化核对。第三，两个例子都在同一个 PDE 族（时间分数阶扩散）里，跨族的普适性未被检验。第四，本篇没有定理，出先验这一情形恰恰是 Theorem 2 那类界最难用的场合——界里的 $\mu(\Gamma^{\perp}_{N}(\epsilon))$ 在真值出先验时初值很大，而它如何衰减仍然只是论证。
...
> $\beta=\min\{1,\ \mathcal L(d,f^{H}(z^{+}))\pi(z^{+})/\mathcal L(d,f^{H}(z^{-}))\pi(z^{-})\}$。
> 本条为阅读者的核对结论，不是原文；引用时建议回查期刊版。

### 定理

**本篇不证明任何定理**，并明确说明沿用编号 37 的分析框架。可核实的设计论断只有一条，而它是前提性的：$Q$ 必须小，因此修正网络的容量必须受限——这是防过拟合的约束。
...
例 2 换成由 Karhunen-Loève 展开参数化的高维随机场渗透率；可核实材料没有单独记录例 2 的网络规模与容差。

**结果。** 固定的先验训练网络给出**可见错误**的后验边缘分布；自适应版本恢复真模型 MCMC 的边缘分布；容差从 $0.1$ 收紧到 $0.05$ 后一致性进一步提高。

> [!note] 这一篇同样没有可转录的数值
> 可核实材料给出的是上表的配置与上述定性对照，**没有误差数值、有效样本量或 CPU 时间**。容差 $0.1\to0.05$ 使一致性提高这一点是定性描述的，没有配套的数值指标。因此本页不为编号 49 列结果表。

**实验建立了什么，又差在哪里。** 建立的是消融意义上的结论：同一条链、同一个初始代理，唯一的差别是在线细化开还是关，开着的那条能对上真模型 MCMC 的边缘分布，关着的那条对不上。这直接把改进归因到细化机制，而不是网络架构或训练技巧。差的地方在于：论文的动机是高维（多项式的基函数爆炸），而例 1 只有 9 个参数，例 2 的维数在可核实材料中没有给出；没有任何量化指标，因此无法比较不同容差、不同 $Q$ 之间的取舍；复合嵌套随外循环加深，其影响完全未被测量。
...
论文明确拒绝它作为**可实现**的指标：它需要一个高维积分。这一步值得保留，因为它把后面所有单点指标的地位说清楚了——它们是这个量的廉价替身，不是它的估计。
...
取 $A=G$、$B=\widehat G$，两项各以 $H\|\Sigma_\eta^{-1}\|_2\epsilon$ 为界即得。随后由反向三角不等式得到 $C_2$ 的存在性。（恒等式与推法是本页补出的常规步骤；论文只叙述结论并抽象地断言某个 $C_2>0$ 存在。）
...
热源反演 Case I 的细节可核实：两参数源位置问题，DeepONet 在 $[0.5,1]\times[0.5,1]$ 上用 500 个均匀样本训练，无迹 Kalman 反演从 $[0.6,0.6]$ 出发；Case II 是更高维的变体。

**关键的实验设计是分布内与分布外两组真值。** 分布外的真值就是把「代理在后验所在处不可靠」这一失效模式显式制造出来，也正是固定代理失效的场合。

**结果（定性，可核实材料未给出数值表）。** 固定代理只给出粗略估计，并产生可见错误的反演轨迹；自适应版本随细化单调降低模型误差，并接近全阶模型精度。第三个例子中细化在六轮后由停机准则终止——这是停机准则确实会触发而不只是被 $I_{\max}$ 截断的证据。
...
| 数值结果 | 仅配置与定性结论       | 完整误差与求解次数     | 仅配置与定性结论       | 仅配置与定性结论          |

## 覆盖核对

| 内容                                   | 论文   | 覆盖状态                                                                  |
| -------------------------------------- | ------ | ------------------------------------------------------------------------- |
| 核对层级与残留缺口                     | 四篇   | 逐篇来源、层级与未核实项                                                  |
| 加权最小二乘、Christoffel 权与行归一化 | 37     | 权的定义、行范数恒为 $\sqrt M$ 的推导、与最优采样专题的同一性             |
| 多精度修正的指标结构                   | 37     | 修正项、按指标合并只是重排、样本预算与过拟合约束                          |
| 算法采的是代理后验这一事实             | 37     | 子链的角色、真模型出现在何处、与渐近精确路线的区别                        |
| $\epsilon$-可行集与 KL / Hellinger 界  | 37, 79 | 假设、结论、推论、界的两项与两个区域的对应                                |
| 正则化集合 Kalman 反演的完整更新式     | 34     | 扩张状态归约、更新、协方差、正则化选择、停机准则                          |
| 线性情形的 Tikhonov–Phillips 极限      | 34     | 与线性高斯后验均值的一致，以及无导数这一代价结构                          |
| 出先验真值下的定量对比                 | 34     | 两个例子的误差、求解次数、CPU 时间与预算公式的一致性核对                  |
| 复合多精度网络                         | 49     | 网络耦合形式、训练集构造、与线性修正的对比、容量约束链                    |
| 接受概率分子分母的核对                 | 49     | 印出形式为何不可能是本意                                                  |
| 目标导向指标与贪心设计                 | 79     | 被拒绝的诚实指标、锚点、相对变化、贪心打分与其代价结构                    |
| 无迹 Kalman 反演与线性收敛定理         | 79     | 预测分析步、$\sigma$ 点、引理 3.5 的恒等式、两个 $O(\epsilon)$ 界与其限定 |
