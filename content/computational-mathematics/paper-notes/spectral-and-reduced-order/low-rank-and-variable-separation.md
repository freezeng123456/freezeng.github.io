...
> [!note] 本页覆盖
> 编号 **15**（_SIAM J. Sci. Comput._ 37(2), 2015）、**17**（_J. Comput. Phys._ 303, 2015）、**83**（_SIAM J. Sci. Comput._ 47(3), 2025）。编号 15 与 83 依据可公开获取的全文逐式核对，本页给出完整推导、定理的全部假设与数值实验设置。编号 17 的期刊正文需订阅访问且无预印本，该节只写摘要与关键词可确认的内容，并把它所改造的通用框架与本文自身的结论分开写——该节明显短于另外两节，这是证据状况决定的，不是遗漏。
...
第三是代价。$\mathcal M_S$ 是弯的，它在一点处的曲率约为 $1/\sigma_S$，即最小保留奇异值的倒数；切空间投影里会显式地出现协方差矩阵的逆。本页后面的每一件事——编号 15 的 $e^{C/\rho}$ 常数、它对奇异协方差的 $C^{\dagger}$ 变通、编号 83 的整个设计动机——都从这一个逆矩阵长出来。
...
> [!note] 编号与版本
> 本节的定理与命题编号取自 MATHICSE 技术报告（预印本题名为 _On the Dynamically Orthogonal approximation of time dependent random PDEs_）；SIAM 出版版本是否重新编号，本站未核实。
...
> 矩阵情形的原始文献给出两条限定，读这一节时值得一并放在心里。其一，若真实的 $\varepsilon$ 伪秩为 $q<r$ 而按秩 $r$ 积分，尽管核矩阵病态、$S^{-1}$ 出现在因子方程中，逼近质量并未受到严重影响。其二，$Y(t)$ 的奇异值合并处并不产生奇性，这一点与光滑奇异值分解的常微分方程不同。真正的失效模式是另一种：秩取得太小时，一个**没有被纳入**逼近的奇异值可以越过正在跟踪的那些而方法察觉不到——这恰好就是本文构造的特征值交叉例子。
...
> [!warning] 本站未能核实的定量结果
> 超出 §5 闭式速率之外的具体误差量级与收敛斜率均来自图形，无法从可获取的文本中还原，因此**本页不给出编号 15 的误差数表**。需要具体数字时应回查原文的图。
...
先前工作从离散化一侧攻这个代价——小波 Galerkin、推广的快速多极、谱元、带张量结构的 Legendre-Galerkin、紧积分算子的多层增广方法——但特征求解本身仍必须在最细网格上完成。本文的贡献正是取消这一点：把特征值问题转化为一列**积分迭代**加上只在**最粗网格**上的特征求解，因此任何高效积分方案都可以插进来，摘要称总工作量与最细网格上的一次积分相当。

### 所改造的通用框架

其所改造的多层修正框架（Lin 与 Xie）的通用一步是：给定最粗空间 $V_H$ 与嵌套层次 $V_H\subset V_{h_2}\subset\cdots\subset V_{h_n}$，先在细空间上解一个**源问题**
...
并归一化 $b(u_{h_{k+1}},u_{h_{k+1}})=1$。第二步的意义在于：特征问题只在维数为 $\dim V_H+1$ 的空间里解，即最粗空间加一个方向，从不在细网格上解。整套方案是：先在 $V_H$ 上解一次特征问题，沿层次循环上述修正步，最后在 $V_{h_n}$ 上再做一次源问题求解并取 Rayleigh 商。

该框架的两条分析结果同样是公开可查的。其一是 Rayleigh 商的误差恒等式：对真特征对 $(\lambda,u)$ 与任意 $0\ne\psi\in V$，令 $\hat\lambda=a(\psi,\psi)/b(\psi,\psi)$，则

$$
\hat\lambda-\lambda=\frac{a(u-\psi,u-\psi)}{b(\psi,\psi)}-\lambda\,\frac{b(u-\psi,u-\psi)}{b(\psi,\psi)} .
$$

**这条恒等式是这一族方法把 $O(\varepsilon)$ 的特征函数误差变成 $O(\varepsilon^2)$ 的特征值误差的原因。** 其二是一步修正对误差的作用与整体结果：
...
其中 $\delta_h(\lambda)$ 是特征函数在 $V_h$ 中的最佳逼近误差，$\eta_a(H)$ 是必须足够小的粗空间「光滑化」量。几何权重 $\eta_a(H)^{n-k}$ 解释了为什么粗空间的贡献不污染最终精度。

对积分算子而言，第一步的「源问题」不是线性求解而是**一次求积**（即 $\mathcal T$ 的一次作用），这正是摘要所说的积分迭代；其自然的形式应当是 $\tilde u_{h_{k+1}}=\Pi_{h_{k+1}}\mathcal T u_{h_k}$，即幂法型的一次作用，后接同样的小增广特征求解。**但这一条是本站按摘要措辞作的重构，不是论文的公式。**

> [!warning] 可核实范围
> 该文正文需订阅访问且无预印本（Elsevier 付费墙，arXiv 无对应预印本，两位作者主页亦无全文）。可确认的是问题设定、关键词（不确定性量化、Karhunen-Loève 展开、Fredholm 特征值问题、多重网格有限元）与摘要中的贡献表述：提供了误差估计、分析了计算复杂度、总工作量与最细网格上的一次积分相当、并有若干数值实验验证效率。上面给出的通用修正步、Rayleigh 商恒等式与误差递推来自被改造的 Lin-Xie 框架，**不能当作本文的定理引用**。
>
> 具体尚未报告的内容有五项：该框架针对积分算子的实际修正步形式；本文误差估计所用的范数、常数与假设；复杂度分析的细节；数值实验采用的协方差核与空间维数；以及所测得的加速比。按其参考文献（Oliveira-Azevedo 的谱元方法、Schwab-Todor 的推广快速多极、Phoon 等人的小波 Galerkin），对照基线合理的猜测是同一积分特征问题的直接有限元或谱离散，但这只是推断，不是核实过的陈述。
...
初值 $u(x,0;\xi)=\sqrt5\,(x_1^2-x_1)(x_2^2-x_2)$，齐次 Dirichlet 边界，$\xi\in[0.1,0.2]$；$h_{x_1}=h_{x_2}=0.05$，$\tau=10^{-4}$，$|\Xi|=8$，$M=10^3$。结果与其余三例不同：平均相对误差先随项数下降，随后**趋于平台**，即出现精度地板。本站未能核实该平台的具体量级，故不给出数表。

> [!note] 题名差异
> 主页把该文列为 _A dynamical variable-separation method for dynamical systems with random input_，而出版版本、预印本与第三方记录均为 _A Dynamical Variable-Separation Method for Parameter-Dependent Dynamical Systems_。本站按出版版本记录；预印本为 [arXiv:2502.08464](https://arxiv.org/abs/2502.08464)。本节的方程与算法编号取自 arXiv 版，SIAM 版是否重新编号本站未核实。
...
| 内容                                           | 论文 | 覆盖状态                                                              |
| ---------------------------------------------- | ---- | --------------------------------------------------------------------- |
| 低秩流形为何是降维而非截断                     | 通用 | 流形状态空间、切空间条件、代价与曲率                                  |
| 动力低秩与动态正交的字典                       | 15   | 五行对应，含 $S^{-1}\leftrightarrow C^{-1}$                           |
| 模型、抛物特化与拟设                           | 15   | 算子形式、三条规范条件、边界与初值                                    |
| 三个演化方程与各自的角色                       | 15   | 均值、基、系数方程及投影算子                                          |
| 切空间投影与 $C^{-1}$ 的来源                   | 15   | 切空间、投影公式、$\sigma=\sqrt{\mathrm{eig}(C)}$、Dirac-Frenkel 形式 |
| 曲率引理                                       | 15   | 两个不等式、常数 $8\rho^{-1}$ 与 $4\rho^{-1}$                         |
| Theorem 4.1 与 $e^{C/\rho}$ 常数               | 15   | 假设、结论、两层读法、各假设进入之处                                  |
| 特征值交叉的闭式失效场景                       | 15   | 两个闭式速率、$e^{6t}$ 之比、与假设的关系                             |
| 三个测试问题                                   | 15   | 设置齐全；误差数值未核实，已声明                                      |
| 奇异协方差的处理                               | 15   | 伪逆的陷阱、改写形式、逐步对角化                                      |
| Fredholm 积分特征问题的结构差别                | 17   | 紧性、谱聚集、稠密矩阵                                                |
| 多层修正步与积分迭代                           | 17   | 通用两步、维数论证、积分与线性求解的差别                              |
| 框架的 Rayleigh 商恒等式与误差递推             | 17   | 明确标注为框架结论，不是本文定理                                      |
| 未报告的五项内容                               | 17   | 修正步形式、常数、复杂度细节、核与维数、加速比                        |
| 三种降阶各自冻结的对象                         | 83   | 多项式混沌、本征正交分解、Kolmogorov 障碍                             |
| 「动态」相对静态分离多出什么                   | 83   | 参数系数的时间依赖及其数值后果                                        |
| 模型、仿射假设与拟设                           | 83   | 结构分解、仿射依赖、去掉均值场与规范                                  |
| 贪心准则与两个解耦子问题                       | 83   | 选点准则、停机、两个方程的完整形式                                    |
| 初值构造与类 Gram-Schmidt 修正                 | 83   | 三个初值公式及其与规范条件的关系                                      |
| 残差驱动与被带着的 $\langle(g_k)_t,g_k\rangle$ | 83   | 线性情形的两个方程与差别落点                                          |
| 标量除数的闭式递推                             | 83   | 递推、$c_{n+1}$、$l_{n+1}$、$s_{n+1}$ 全式                            |
| 离线在线划分与在线代价                         | 83   | 所存七类标量、在线标量方程、与空间离散无关                            |
| 后验界与对数 Lipschitz 常数                    | 83   | 定义、$\delta_k$、可为负及其后果                                      |
| 没有收敛定理与自陈限制                         | 83   | 结论部分的表述与存储依赖                                              |
| 四个数值算例                                   | 83   | 三张结果表、幅度层级表、Allen-Cahn 的平台                             |
...
- 编号 17 一节所引的通用多层修正框架取自 Q. Lin and H. Xie, [_A multi-level correction scheme for eigenvalue problems_](https://doi.org/10.1090/s0025-5718-2014-02825-1), Math. Comp. 84(291) (2014), pp. 71-88（预印本 [arXiv:1107.0223](https://arxiv.org/abs/1107.0223)）。
