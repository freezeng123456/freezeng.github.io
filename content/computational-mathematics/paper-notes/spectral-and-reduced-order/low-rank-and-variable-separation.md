...
编号 83 与编号 15 属于同一族问题：为参数依赖的动力系统构造随时间演化的分离表示，而不是在固定基上展开。出版题名为 _A Dynamical Variable-Separation Method for Parameter-Dependent Dynamical Systems_，与主页所列题名略有差别，本站按出版版本记录。

> [!note] 精读待补
> 本页尚未对该文做逐式核对，因此不报告其分离形式、更新公式与理论结果。可参考的定位是：它与编号 15 共享「让表示随解演化」的取向，而与固定基的降阶模型形成对照。

## 三篇的关系

| 编号 | 演化对象                 | 参照的最优对象         | 主要风险                 |
| ---- | ------------------------ | ---------------------- | ------------------------ |
| 15   | 空间基与随机系数         | 截断 Karhunen-Loève    | 最小奇异值塌缩           |
| 17   | 不演化（一次性特征求解） | 精确 Karhunen-Loève 基 | 稠密矩阵的装配与求解代价 |
| 83   | 分离表示（待补）         | 待补                   | 待补                     |
