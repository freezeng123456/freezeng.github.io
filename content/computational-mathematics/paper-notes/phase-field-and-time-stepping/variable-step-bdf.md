...
>
> 其中 **48、52、58、67、74** 五篇已按作者自己的 arXiv 源文件逐式核对，因此下面的公式、定理假设与常数是转录而非转述，五篇都给出完整的推导链、带假设的定理，以及（若论文本身有）数值实验；编号 58 与 74 是纯分析论文，本身不含数值实验，本页照实说明。
>
> **编号 69 无预印本**——已对 Liao 名下的 34 篇预印本逐一排查确认，Springer、Semantic Scholar 与 zbMATH 均不提供正文。因此本页对它只报告能从完整摘要与 22 条参考文献确认的内容：步长比条件、所用成分（单腿改写、离散梯度结构、两类新的 DOC 核）与三条结果。**其显式修正能量、DOC 核的定义、收敛阶与数值实验本站未核实，不作补写。** 变步长 FiBE 格式本身是从方法出处（DeCaria-Guzel-Layton-Li，[arXiv:1810.06670](https://arxiv.org/abs/1810.06670)）重构的，页面上保持这一标注。
...
最大值原理那一侧要的是**非负性加单调性**，这是更强的要求。理由在本页开头已经说了：$\ell^\infty$ 的归纳只对非负权重有效。BDF2 的核不满足，于是本文做一次**变量替换**：令 $\bar v^k=v^k-\eta v^{k-1}$。这个替换的效果是把一个符号不定的两项核换成一个几何衰减的非负核族 $d^{(n)}_j$，代价是 $\eta$ 必须落在一个区间里，而该区间非空当且仅当 $r_k<1+\sqrt2$。
...
> BDF2 核正定性的门槛后来被改进到 $r_*\approx4.864$（$1+2r-r^{3/2}=0$ 的正根，精确值 $4.864536512317583$），但这个改进**不属于本专题的任何一篇**：在十篇可得原文中检索 `4.86` 无命中。它出自 Liao、Ji、Wang 与 Zhang（_J. Sci. Comput._ 92 (2022) 52，预印本 [arXiv:2102.03731](https://arxiv.org/abs/2102.03731)）——一篇**没有周涛署名**的论文，那里印的是 $4.864$，流传更广的 $4.8645$ 写法出自 Zhang 与 Zhao（_J. of Math._ (PRC) 41(6) (2021) 471-488）。编号 69 与 91 都引用了 Liao-Ji-Wang-Zhang，这大概就是这个常数进入本阅读清单的路径。完整的辨析见[[computational-mathematics/paper-notes/phase-field-and-time-stepping/index|本专题首页]]。此外要注意：$4.8645$ 改进的是 $3.561$（同一条正定性引理），而**不是**取代 $1+\sqrt2$——后者管的是另一件事。
...
**这样的 $\eta$ 存在当且仅当 $\dfrac{r_k^{2}}{1+2r_k}<1$，即 $r_{k}^{2}-2r_{k}-1<0$，即 $r_{k}<1+\sqrt{2}$。** 这正是条件 S0，也正是 Grigorieff 的经典零稳定性界。（$r^2-2r-1=0$ 到 $r=1+\sqrt2$ 这一步是本站从原文印出的条件 $\frac{r_k^2}{1+2r_k}\le\eta<1$ 补出的初等算术；结论 $r_k<1+\sqrt2$ 及其与 Grigorieff 条件的一致性是原文明写的。）
...
> [!warning] 算例三的初值在源文件中自相矛盾
> arXiv 源把粗化算例的初值写作 $u_{0}=0.95+\mathrm{rand}(\mathbf x)\times0.05$，同时又说随机数「从 $-0.05$ 变化到 $0.05$」。这两句不能同时成立（前者给出均值 $0.95$ 的初值，后者暗示以 $0$ 为中心的扰动）。本站**未能确定**实际使用的初值，因此这里只报告网格与参数，不报告初值的具体形式。
...
性质 2 把与 $1+\sqrt2$ 的关系显示得一清二楚：**乘积 $\prod_i\frac{r_i^2}{1+2r_i}$ 衰减当且仅当每个因子小于 $1$，即 $r_i<1+\sqrt2$**（这一句连线是本站的观察，闭式本身是原文的）。性质 3 则是把空间相容误差转成 $t_nh^2$ 的那一步。
...
**这张表是本页最有说服力的一处理论-实验落差记录。** 结论很直接：即便有几十个时间层的步长比**大幅**违反 $3.561$（最大处 $850.80$，是门槛的两百多倍），二阶收敛依然保持。也就是说 **S1 是充分条件，远非必要条件**；变步长 BDF2 在经验上比理论保证的稳健得多。论文自己也把 $r_s=3.561$ 称为「由条件 S1 带来的人为常数」，这张表就是这句自评的证据。
...
论文只显式给出 $c_{\mathrm I,3}=11/7$；$c_{\mathrm I,4}$ 与 $c_{\mathrm I,5}$ 只断言有限（做法是把 $n=4,5$ 与 $n=5,6,7$ 这有限几个低指标情形单独吸收进常数）。**它们的具体数值原文未给出，本站不作补写。**
...
代入 $\rho_k$ 的值，这个限制读作 $\tau\le6/(35\beta^{*})$（$k=3$）、$\tau\le1/(14\beta^{*})$（$k=4$）、$\tau\le2/(134.4\,\beta^{*})$（$k=5$）——这三个数是本站按公式 $\tau\le(7-k)/(7\rho_k\beta^{*})$ 算出的，原文只印符号形式。
...
**本文没有数值实验。** 它是一篇纯分析论文，源文件里仅有的两组图形是：(a) $3\le k\le5$ 时生成函数 $\mathrm g^{(k)}(\varphi)$ 在 $[-\pi,\pi]$ 上的曲线，用来显示 $\mathrm g^{(k)}_{\min}>0$；(b) 计算出的 DOC-$k$ 核与界 $\frac{\rho_k}{4}(k/7)^{x}$ 的对照图，用来显示几何衰减。
...
> [!warning] 本节的证据基础与其余各篇不同
> 编号 69 **没有 arXiv 预印本**（已对 Liao 名下的 34 篇预印本逐一排查确认），Springer、Semantic Scholar 与 zbMATH 均不提供正文。本站取得的只有**完整逐字摘要**与 **22 条参考文献**。因此下面明确区分两类内容：可核实的（步长比条件、所用成分、三条结果的**存在与类型**），与**未核实**的（显式修正能量、两类 DOC 核的定义、收敛阶、数值实验的设置与结果）。**未核实的部分本站不作补写，也请勿据此引用。** 格式本身是从方法出处重构的，见下。
...
以下格式取自 DeCaria-Guzel-Layton-Li（[arXiv:1810.06670](https://arxiv.org/abs/1810.06670)，第 2 节的 Example，那里明确引用 Guzel-Layton 第 307 页），**不是**从编号 69 本身取得的。以 $\tau=k_{n+1}/k_n$ 记当前步与前一步之比：
...
其左端正是变步长 BDF2 差分算子。**这是编号 69 所利用的结构事实**：OLM 改写的存在及其 BDF2 左端由方法出处核实，编号 69 确实使用它则由摘要用词核实。

### 可核实的结果

三条结果的**存在与类型**由摘要核实；其显式常数与假设未核实。

1. **离散能量耗散律。** 为变步长 FiBE 所对应的单腿多步格式建立了离散梯度结构，并在**耗散情形**下建立离散能量耗散律。**修正能量泛函的显式形式未核实。** 按编号 48、52、67 的类比，它大概会是「$H^1$ 半范加一个关于 $\partial_\tau u^n,\partial_\tau u^{n-1}$ 的非负二次修正」的形状，但这只是推测，不能当作事实报告。
2. **$L^2$ 稳定性。** 在下述步长比条件下严格成立。常数未核实。
3. **$L^2$ 收敛性。** 在同样条件下的严格误差估计。摘要声明优先性：「这似乎是变步长时间滤波刚性求解器的**第一个**能量稳定性与 $L^2$ 范数误差估计。」**收敛阶在摘要中没有给出。** FiBE 按构造是二阶的（Guzel-Layton），因此关于 $\tau$ 的二阶估计是自然的预期，但**确切的阶与常数形式未核实**。

分析装置方面，摘要说离散能量技术是配合**两类新的离散正交卷积核**发展的。**这两族核的定义及其各自满足的恒等式未核实。** 「两类」这个复数与 OLM 结构是相容的——左端的 BDF2 卷积与右端的单腿／插值卷积各需要自己的正交对偶——但这是本站的解读，不是论文的陈述。

本文处理的是**线性抛物方程**，不是 Allen-Cahn，因此**没有**最大值原理的结论（摘要写明「线性抛物方程」，zbMATH 的 MSC 是 65M06、65M12、65M15、65N35、35K05、35Q79，没有反应扩散或最大值原理的类别）。
...
> [!warning] $[1/2,\,2]$ 与本页其余门槛不是同一类对象
> **第一，它是双边的。** 本页其他所有结果都只限制步长比的**上界**（$r_k<1+\sqrt2$、$r_k<3.561$、$r_k<1.4877$），而这里步长比还被 $1/2$ 从下方限制——也就是说步长既不能放大太快，也不能**收缩**太快。这反映的是变步长 FiBE 失去了 $A$-稳定性。
>
...
摘要只说「我们也给出数值实验以支持理论结论」。**具体的试验问题、网格与观测阶本站未核实**，因此本页不报告任何数值。这是编号 69 与本页其余五篇之间最大的缺口。
...
**单腿改写是通往 BDF2 各篇的桥**：FiBE 所对应的 OLM 左端恰好是变步长 BDF2，因此编号 48、52 的梯度结构机器可以直接转移，只是非线性项与外力在一个滤波过的宗量处求值。它的步长比结论则是本页的**异类**：双边且经验，而非单侧的尖锐代数门槛，这正反映了变步长 FiBE 失去 $A$-稳定性这一事实。
...
再由 **C4** 补上余下的单调性，即可断定 $\theta$ 正定。（引理 2.7 第二条在 **C4** 下给出的结论，其**确切措辞本站未核到**——核对时源文本在该处被截断；其结构作用如上所述。）定理 1.1 就是把 DCC 单调性与引理 2.1 的 DOC 等价性合起来。
...
**本文没有数值实验。** 它是纯分析论文，源文件里唯一的图形是核的示意图。因此它给出的所有结论都是定理，没有实测数据可以对照——这一点与编号 58 相同。
...
| 69   | 线性抛物，滤波 Euler  | 变步长 | $[1/2,2]$（双边，经验）   | 单腿改写；两类新 DOC 核（定义未核实） | 存在但设置与结果未核实 |
| 74   | 二次型本身            | 任意   | 无（给出代数判据 C1-C4）  | DOC 与 DCC 的一般理论        | 无                           |

**读这张表的方式**：门槛一列的数值大小不可横向比较——它们分别是不同格式、不同结论的条件。真正可比的是最后一列：**每当论文自己做了随机网格实验，观测到的稳健范围都远宽于定理保证的范围**（编号 52 的 $850.80$、编号 67 的半数层违反），这是本页最一致的经验事实。本页六篇里没有一篇给出打破结论的反面算例；唯一那个能把最大值界真的打破的实验在[[computational-mathematics/paper-notes/phase-field-and-time-stepping/time-fractional-phase-field|编号 43]]，而它破坏的是**步长-空间耦合条件**而非步长比条件。两相对照说明：耦合条件比步长比条件更接近必要。

## 覆盖核对

| 内容                                    | 论文 | 覆盖状态                                                 |
| --------------------------------------- | ---- | -------------------------------------------------------- |
| 为何符号不定的核挡住能量与最大值原理    | 全页 | 开篇机制段：DOC 求逆／DCC 求补／KRC 换基                 |
| 变步长 BDF2 公式与核的符号              | 48   | 公式、卷积核、$b^{(n)}_1<0$ 的含义                       |
| 条件 S0 与 S1 及各自的性质              | 48   | 两个常数、两条性质、与 Grigorieff 的一致性               |
| 修正能量与前瞻项                        | 48   | 形式、$O(\tau)$、为何用 $r_{k+1}$                        |
| $3.561$ 的精确推导与步长上界分段        | 48   | 下界不等式、正根、$h(x)$ 单调性的三段讨论                |
| 核重排、最优 $\eta$ 与 $1+\sqrt2$       | 48   | $\eta$ 变换、闭式核、$K(\eta)$、$r_s=1,2$ 的算例         |
| 最大值原理的完整论证                    | 48   | $Q^{(n)}_j$ 非负、立方引理、辅助断言 $\le1-\eta$         |
| DCC 核与新的 Grönwall 不等式            | 48   | 定义、互补恒等式、实用界、交换求和的一行证明             |
| 五条定理与三个算例                      | 48   | 可解性、能量、最大值原理、Grönwall、最大范数收敛；初值不一致已标注 |
| $4.8645$ 的归属                         | 48   | 不属于本专题；出处、印法差异、与 $1+\sqrt2$ 管不同的事   |
| 无斜率选择模型与它为何更难              | 52   | 对数能量、无最大值原理、四阶、与编号 78 的区别           |
| DOC 核定义、正交恒等式与矩阵解释        | 52   | 递归、$\equiv\delta_{nk}$、$\mathbf\Theta_2=\mathbf B_2^{-1}$ |
| DOC 核闭式与三条性质                    | 52   | 乘积公式、与 $1+\sqrt2$ 的联系、$\sum\theta=\tau_n$      |
| 二次型正定性与 $\mathcal M_r$           | 52   | 逐层不等式、$\mathcal M_r<39$ 与 $\le4$                  |
| 四条定理                                | 52   | 可解性、能量、$L^2$ 稳定性、$L^2$ 收敛与起步误差阻尼     |
| 随机网格实验与门槛的非必要性            | 52   | $\max r_k$、$N_1$、观测阶三列；$850.80$ 仍二阶           |
| 乘子技术的替代与可逆变换                | 58   | Nevanlinna-Odeh 的代价、DOC 变换、经典能量不等式         |
| 生成函数、Grenander-Szegő 与 $\sigma_k$ | 58   | $\mathrm g^{(3,4,5)}$、配方、三个显式常数与极小点        |
| DOC 核几何衰减与启动项                  | 58   | $\rho_k$、公比 $k/7$、$7/(7-k)$、$c_{\mathrm I,3}=11/7$  |
| 三条定理与「无数值实验」                | 58   | 两条稳定性、$k$ 阶收敛；仅两组引理示意图                 |
| Calvo-Grigorieff 前因子的不稳健         | 67   | 交替网格反例与 $\Gamma_M$ 的发散                         |
| 变步长 BDF3 与其 DOC 核                 | 67   | $d_0,d_1,d_2$、DOC 递归、双向正交性、等价卷积格式        |
| $1.4877$ 的来历与尖锐程度               | 67   | $\gamma=7/10$ 的取舍、$0.6924/1.4965$、最小特征值表      |
| 离散梯度结构                            | 67   | $d_*,p,q,G,F$ 与常数 $1/50$                              |
| 四条定理与三阶算例                      | 67   | 能量、两条 $L^2$ 稳定性、三阶收敛；两族网格的观测阶      |
| 时间滤波格式（重构）与单腿改写          | 69   | DGLL 原式、Liao 记号、$r_n=1$ 检验、OLM 的 BDF2 左端     |
| 可核实与未核实的分界                    | 69   | 三条结果的类型可核实；修正能量、核定义、阶、算例未核实   |
| $[1/2,2]$ 为何不同类                    | 69   | 双边、经验保护区间、失去 $A$-稳定性                      |
| C1-C4 判据与两套核的一般理论            | 74   | 四条件的含义、DOC／DCC 定义、两条恒等式、闭式            |
| 四个应用与尖锐性                        | 74   | L1、时间分数阶 Allen-Cahn、RL 积分、Volterra；L1$^{+}$ 反例 |
...
- H.-l. Liao, T. Tang, and T. Zhou, [_Stability and convergence of the variable-step time filtered backward Euler scheme for parabolic equations_](https://doi.org/10.1007/s10543-023-00982-y), BIT Numer. Math. 63 (2023), 39。**无预印本**；本页对它只报告可从摘要与参考文献核实的内容。
...
- H.-l. Liao, B. Ji, L. Wang, and Z. Zhang, [_Mesh-robustness of an energy stable BDF2 scheme with variable steps for the Cahn-Hilliard model_](https://doi.org/10.1007/s10915-022-01923-7), J. Sci. Comput. 92 (2022), 52（预印本 [arXiv:2102.03731](https://arxiv.org/abs/2102.03731)）——$4.864$ 的出处，**无周涛署名**。
