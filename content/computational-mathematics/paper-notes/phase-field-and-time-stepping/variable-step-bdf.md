...
- **S1**：$0<r_{k}<\dfrac{3+\sqrt{17}}{2}\approx3.561$，用于**能量稳定性**。
...
**编号 69** 分析变步长时间滤波后向 Euler 格式的稳定性与收敛性。时间滤波是在低阶格式上加一个后处理以提高精度的技术，而变步长下它的稳定性同样落回这一套卷积核分析。
...
| 69   | 抛物方程，滤波 Euler  | 变步长 | 见原文                    | 同族卷积核分析         |
