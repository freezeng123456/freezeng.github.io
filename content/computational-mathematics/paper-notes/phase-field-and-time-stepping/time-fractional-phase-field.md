...
> [!note] 本页覆盖
> 编号 **40**（_SIAM J. Sci. Comput._ 41(6), 2019）、**43**（_J. Comput. Phys._ 414, 2020）、**57**（_SIAM J. Sci. Comput._ 43(5), 2021）。三篇均已按作者自己的 arXiv 源文件逐式核对，因此下面的方程、定理条件与常数是转录而非转述；三篇都给出了完整的推导链、带假设的定理与数值实验。
>
> 三篇构成一条清晰的推进线：编号 40 证明积分型能量律并明确指出它不能被加强，编号 43 在非均匀网格上补上二阶保最大值原理的格式，编号 57 换掉能量对象、换来一条微分型的律。
...
（原始 LaTeX 源在积分限处把 $\tau$ 误排为 $\pi$；右端的闭式无歧义，实际使用的是它。）这些核满足
...
> [!warning] 一处疑似源文件排印
> 在作者的接收版 LaTeX 源中，分子束外延定理里 $A_\alpha$ 的两个宗量写作 $(\phi,\phi)$，而按 Allen-Cahn 与 Cahn-Hilliard 两条的类比以及证明结构，应为 $(\phi_t,\phi_t)$。本站判断这是源文件的排印疏漏，但**未核对 SIAM 正式出版版本**，因此引用这一显式时必须回查出版 PDF。
...
（第 1 条的下界在 arXiv 源中印成了 $\frac{4}{11\tau_n}\int_{t_{n-1}}^{t_n}$，与核的指标 $n-k$ 不匹配；按 Grönwall 引理的假设应为 $\frac{4}{11\tau_k}\int_{t_{k-1}}^{t_k}\omega_{1-\alpha}(t_n-s)\mathrm ds$。本站判断这是指标排印疏漏，常数 $4/11$ 本身与 $\pi_A=11/4$ 一致。）
...
（源文件把前因子写作 $1/p_0^{(j)}$，应为 $1/A_0^{(j)}$。）由核的单调性 $P^{(n)}_{n-j}\ge0$，并且
...
（$G_{\mathrm{loc}}$ 第二项在源文件中积分限写反，此处按显然的意图写正。）由于 $P$ 与 $A$ 互补，全局误差是 $\sum_jP_{n-j}^{(n)}|\Upsilon^j[v]|$，它远小于 $\max_j|\Upsilon^j[v]|$：在 $n=1$ 且 $\sigma=\alpha$ 时 $|\Upsilon^{1}|\le C_u\tau_1^{\sigma-\alpha}/\sigma$ 只是 $\mathcal O(1)$，而 $P_0^{(1)}|\Upsilon^1|\le G_{\mathrm{loc}}^1\le C_u\tau_1^{\sigma}/\sigma$。**这就是「借互补核实现超收敛」，也是估计能够反映时间正则性而不是最坏截断误差的原因。**
...
| 连续核正定性的完整证明路线           | 40   | 对称化、Liouville 半群、Fourier 变换、常数 $\cos/\sin$ 的来源              |
| 积分型而非微分型，及其不可加强       | 40   | 结论形式、原文警告、对编号 57 的影响                                       |
| L1 核、重排恒等式与离散二次型正定性  | 40   | 核公式、完全单调性、$M$-矩阵比较、$s_n$                                    |
| 连续与离散能量定理及其条件           | 40   | 三个模型、稳定化条件、$\lambda_{\max}\le1/8$、$S\ge\gamma/(16\varepsilon)$ |
| 离散最大值原理与其步长条件           | 40   | $(b_0-b_1)/\tau+S$ 条件与它为何非无条件                                    |
| 数值实验与 $-\alpha/3$ 幂律          | 40   | Fourier-Galerkin、SOE、三个算例、经验性说明                                |
| 步长比约定的倒数关系                 | 43   | $\rho_k=1/r_{k+1}$ 与引用风险                                              |
| Alikhanov 公式、两指标核、三条核估计 | 43   | $a,b,A$ 的定义、$\pi_A=11/4$、印刷疏漏标注                                 |
| DCC 核、互补恒等式与分数阶 Grönwall  | 43   | 定义、$\equiv1$、幂型估计、Grönwall 全式                                   |
| 唯一可解性、最大值原理、尖锐收敛     | 43   | 三条定理的完整假设与结论                                                   |
| 误差的卷积结构与超收敛机制           | 43   | $G_{\mathrm{loc}}$、$G_{\mathrm{his}}$、$n=1$ 的对比                       |
| 自适应策略与三个算例                 | 43   | SOE 容差、守卫、阶表、反面算例                                             |
...
| DOC 核与可逆变换                     | 57   | 正交性、互补性、单调性（引用编号 74）、等价的 Caputo 形式                  |
| 四条定理与四个算例                   | 57   | 无条件能量律、渐近保持、可解性、最大值原理；$E$ 非单调的图像               |
