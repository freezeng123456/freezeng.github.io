...
> [!warning] 可核实范围
> Karhunen-Loève 结构参考场的代数形式、流的耦合层形式与似然加物理的组合损失，本页依据的核对材料未能逐式确认。上面三步是论文自己的表述，可以放心引用；但**具体公式不在本页给出**，需要时应直接阅读原文第 3 节。

### 定理

摘要不宣称任何定理，核对材料也未在正文中找到收敛或逼近结果。这一点值得明说：本文的贡献是表示与训练框架，不是分析。
...
摘要的定性结论是模型能够学习非高斯过程并求解若干类随机 PDE。

> [!warning] 数值结果的可核实范围
> 三组实验的构成已核实，**误差量级未核实**，因此本页不给数字。
...
**损失。** 训练信号是时间依赖 Fokker-Planck 残差，没有标签数据。

> [!note] 损失的离散形式
> 具体的离散残差表达式在本页依据的核对材料中是**从散文重建的**，而不是逐式抄录：它是在时空配点 $(\bm x^{(i)},t^{(i)})$ 上取的均方 Fokker-Planck 残差，与 ADDA 一支的做法一致。引用精确形式前建议回查原文。

### 定理

本文不宣称定理。摘要的主张是方法论层面的：格式无网格、不需要标签数据、易于推广到高维。
...
> [!warning] 数值结果的可核实范围
> 三类实验的构成已核实；**具体维数与误差量级未核实**，自适应刷新配点的精确时间表也未能确认，因此本页不给数字。
...
> [!warning] 一处记号冲突
> 预印本渲染中的这个耦合层公式把 $\tanh$ 的尺度界写成 $\alpha$，而 $\alpha$ 在全篇是分数阶指数。应读作 $\beta$，与编号 64 的对应公式以及原始 KRnet 层一致。
...
施加的对象与编号 66 不同：那里是通用 PINN 代理，这里是一个**密度**，因此非负性与归一化是由流保证的，而不需要额外约束。
...
> [!note] 与编号 66 的一处细微差别
> 编号 72 在内层与外层分别使用两个**独立**的方向变量 $\bm\xi$ 与 $\bm\eta$，而编号 66 的对应公式复用同一个 $\xi$。差别很小但确实存在。
...
至于摘要之外常被转述的两句话——「MCNF 与 GRBFNF 相对非自适应方法把精度提高至少一个量级」，以及「GRBFNF 在低维更好、MCNF 在高维更好」——**本页依据的核对材料未能在可访问文本中确认这两句的措辞与数量级**。后者是两种构造的自然读法（径向基中心数随 $d$ 变差，Monte Carlo 不会），但应当当作待核实的假设，而不是论文的结论。
...
这正是摘要所说的「交替细化训练集与近似解」。它与[[computational-mathematics/paper-notes/scientific-machine-learning/adaptive-sampling-for-pinns|编号 70]]那一支的区别在于所依据的量：那里是残差失效概率，这里是**解密度本身**。
...
> [!warning] 数值结果的可核实范围
> 四类算例的构成已核实；**具体维数（如 4/6/8 维）与误差量级未核实**，因此本页不给数字。
...
摘要与结论的定性主张是：B-KRnet 生成的样本与真值「高度吻合」，自适应密度逼近格式在四维问题、Keller-Segel 方程与动理学 Fokker-Planck 方程上都有效。论文自陈的限制有两条：B-KRnet 的光滑性只到一阶，要提高需要更高次多项式；在深度 Ritz 框架内做自适应点更新留作开放问题——后者由[[computational-mathematics/paper-notes/scientific-machine-learning/adaptive-sampling-for-pinns|编号 80]]接手。

> [!warning] 数值结果的可核实范围
> 六个算例、共同设定与两条自陈限制均已核实；**具体误差数值未核实**，因此本页不给数字。
...
| 内容                                | 论文 | 覆盖状态                                                                 |
| ----------------------------------- | ---- | ------------------------------------------------------------------------ |
| KL 结构与流的分工                   | 62   | 直觉、三步构造、三条自陈优势                                             |
| 62 的数值实验                       | 62   | 三组实验；**公式与误差量级未核实**                                       |
| 时间条件化与 Jacobian 退化          | 64   | 三条备选路线、错误做法、正确做法与后果                                   |
| 时间依赖仿射耦合层                  | 64   | 公式、$\beta$ 与 $t$ 输入的作用、与 real NVP 的两点差别                  |
| 64 的数值实验                       | 64   | 四步算法与三类问题；**维数与误差未核实**                                 |
| Lévy 噪声下的分数阶 FP 方程         | 72   | 方程、密度模型、耦合层与记号冲突                                         |
| Lemma 3.1 与 Beta 集中的稳定性规则  | 72   | 完整表示、两个独立方向、$r_\epsilon$ 依赖                                |
| Lemma 3.2、GRBF 与耦合损失          | 72   | 闭式公式、混合模型、一致性罚的必要性                                     |
| 乘 $t$ 使初值条件构造性成立         | 72   | 耦合层形式与其含义                                                       |
| 72 的数值实验                       | 72   | 自适应四步、四类算例；**维数与误差未核实**；两句常被转述的结论标为待核实 |
| 分段线性密度与分段二次 CDF          | 87   | 逐段积分、稳定的逆、重参数化与数值常数的作用                             |
| 伪三角结构、外层冻结与 $d_1=1$ 情形 | 87   | 块分解、压缩操作、边界情形的理由                                         |
| Prop. 2.4 与 Prop. 2.6              | 87   | 两条命题的陈述与证明状态                                                 |
| 一阶系统改写与三项损失              | 87   | 引入 $\bm g$、三项损失、构造性满足的两个性质                             |
| 87 的数值实验                       | 87   | 六个算例、共同设定、自适应四步、两条自陈限制；**误差未核实**             |
