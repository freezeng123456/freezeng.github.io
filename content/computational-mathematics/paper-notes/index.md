...
本专题系统整理周涛（中国科学院数学与系统科学研究院计算数学与科学工程计算研究所）在个人主页上列出的全部 108 项工作，其中 95 项已发表、13 项处于投稿或预印本状态。编号与主题划分依据[主页发表列表](https://lsec.cc.ac.cn/~tzhou/)。

> [!info] 本专题的定位与边界
> 这些页面是**第三方阅读笔记**，不代表作者本人的表述。每页给出问题背景、方法构造、主要结论与限定条件，完整证明请回到原文。
...
- 想了解整体思路：先看上面那三个反复出现的技术动作，再看任一专题的索引页。
...
逐篇读的时候，下面三点都不明显；把整个语料一次读完，它们反复出现，而且都影响「这条结果能不能直接拿来用」。
...
编号 8 对 $k=6$ 给出两个互相矛盾的收敛率（5.116 与 6.273，取决于网格范围）；编号 25 的算例一在 $Y$ 分量上给出 2.632，与其三阶断言不符；编号 93 自称精度不及 PINN 与 RS-PINN，但其表格在 $d=10^5$ 的 Allen-Cahn 上胜过 RS-PINN；编号 105 声称平滑一致优于滤波，而其 Table 9 在 $K=50$ 处相反。

## 推荐系统方向的两篇精读

同样按「问题—机制—取舍」组织，但对象是工业推荐系统而非计算数学，因此单列。

- [[computational-mathematics/paper-notes/推荐系统/长序列建模|推荐系统长序列建模]]：检索、压缩、层次化、稀疏交互与线性模型五条路线，如何在延迟、显存与信息保真度之间取舍。
- [[computational-mathematics/paper-notes/推荐系统/视频号生成式推荐大模型|视频号生成式推荐大模型]]：把召回、排序与重排合并为一个生成式模型的全链路设计，含语义 ID、强化学习目标、指令跟随与序列集合建模。
