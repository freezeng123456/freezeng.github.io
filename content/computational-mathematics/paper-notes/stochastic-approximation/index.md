...
（归一化的）Christoffel 函数就是它的倒数 $N/K(z)$，加权最小二乘的权则一律写成 $1/K$ 的形式。文献两种记法都有：有的把 $\sum_\alpha\varphi_\alpha^2$ 本身叫作 Christoffel 函数，有的叫它的倒数；以下取前者为 $K$、后者为 Christoffel 函数，引用具体论文时需核对该文自己的约定。

写成向量形式 $K(z)=\varphi^{\mathsf T}\varphi$ 立刻可见：任何正交变换 $\psi\leftarrow U\varphi$ 都不改变 $K$，因此**它是子空间 $\mathbb P_\Lambda$ 的性质，与基的选取无关**。一个不依赖基的量才配做这一族工作的中心对象。
...
- **高维低次时诱导采样的优势会缩小**，因为低次空间使 $\rho$ 接近 $w$；编号 45 在 $d=4$、双曲叉指标集的 PDE 算例上只看到不大的优势。诱导采样始终名列前茅，并且是唯一带有最小样本数定理的方案，但并不总是数值上最好的那一个。
