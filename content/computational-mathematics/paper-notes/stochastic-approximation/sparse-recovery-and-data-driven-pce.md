...
在稀疏恢复框架下，设计矩阵被要求满足有界正交系下的限制等距性质，而所需测量数由行范数的均匀性控制——**与[[computational-mathematics/paper-notes/stochastic-approximation/optimal-sampling-and-preconditioning|最小二乘情形]]完全相同的量**。这就是这六篇与那条线索共享 Christoffel 权与预条件语言的原因。

六篇论文可以按「用什么当行、用什么当列、行怎么归一化」来读：
...
**Christoffel 记法。** 再生核对角线记为 $K(z)=\sum_{\alpha\in\Lambda}\varphi_\alpha^2(z)$，Christoffel 函数是它的倒数 $\lambda_\Lambda(z)=1/K(z)$，归一化 Christoffel 函数是 $N/K(z)$，$N=|\Lambda|$。因此「Christoffel 权」一律指 $1/K$；最小二乘目标里出现的 $N/K$ 与 $1/K$ 只差一个与 $z$ 无关的常数，不改变极小元。原文各家的符号并不一致，这是读这一族论文时的主要混淆来源：编号 36 用符号 $K$ 表示 $N/\sum_j\Phi_j^2$（即归一化 Christoffel 函数本身），编号 44 用 $\kappa$ 表示 $\frac1N\sum_j\Phi_j^2$（即倒数）。下文引用原式时一律换算到上面的记法。

**样本数与基函数个数。** 编号 10、21、36、44 采用「样本数在前」：$m$ 或 $M$ 个样本、$N$ 个基函数；编号 29、32 恰好相反：$N$ 个样本、$M$ 个基函数。每一节沿用该论文自己的记号并在节首标明，跨节比较时须留意。另外编号 29 的 $\Gamma$ 指频率指标集 $\Gamma\subset\mathbb Z^d$，而编号 21、32 的 $\Gamma$ 指参数域。
...
Legendre 多项式**不是**一致有界的，但乘上 $(1-x^2)^{1/4}$ 之后是；保持加权系正交性的偏置测度恰是 Chebyshev 测度。这就是 Rauhut–Ward 策略，也是下面反复出现的**倒数配对**原则：按 $\propto w/(N\lambda_\Lambda)$ 的密度采样，再用 $N\lambda_\Lambda$ 做预条件，使复合系一致有界。编号 21 用 Gauss 权实现它，编号 32 用 $\sqrt{\rho^{(\alpha,\beta)}/\rho_c}$ 实现它，编号 44 用 $\mu\propto\kappa$ 采样、$1/\kappa$ 加权实现它。
...
作者报告的结论是：**确定性点集与随机点表现相当**（论文的措辞是 "a similar performance"）。也就是说，实验建立的不是精度上的优势，而是「确定性并不需要付出精度代价」这一点。

实验与理论之间还有一道落差：定理 4.3 要求 $m\gtrsim4^dd^2s^2$，而实验实际使用的点数远小于这个量级。**这说明那条界是充分条件而不是紧的**，实验检验的是方法在理论保证之外的实际表现。
...
**即 Gauss 权恰好是 Christoffel 函数值**（即 $1/K$ 的张量形式）。网格上的均匀经验概率测度是
...
> $$
>
> 即「概率**不超过** $1-N^{-\gamma\log^3 s}$」。这个方向是不可能的：它对任何一个没有内容的界都自动成立，而与「以高概率成立」的本意相反，也与同一篇论文的定理 2.2（"with probability at least $1-N^{-\gamma\log^3(s)}$"）自相矛盾。正确的方向是 $\Pr[\cdot]\ \ge\ 1-N^{-\gamma\log^3(s)}$，上面的陈述已按此写出。
...
实验与理论之间的落差在高维一栏最明显：定理 4.1 在 $d=10$ 时要求的 $M$ 含有 $C^{10}$ 这样的因子，而实验用的是 $M=85$。**实验说明的是方法在远小于理论样本数时仍然可用，而不是那条界是紧的。** 论文对 $C=2$ 是否成立也只说图 1「提示」，没有下结论。
...
（本节记号沿用论文：$N$ 个采样点，$M=\#\Gamma$ 个基函数，$\Gamma\subset\mathbb Z^d$ 是频率指标集，$s$ 为稀疏度。论文用 $k$ 同时表示方向个数与频率多重指标；下文把方向个数记为 $r$ 以免混淆。）
...
把它用到增广向量 $x=(1,V^\top k)$ 与 $y=(1,V^\top k')$ 上：可容许性保证二者不成比例，因而存在满足两个约束的 $u$；再把 $\|y\|^2$ 与 $\|u\|^2$ 在 $\Gamma$ 上一致地放大，就得到定理 2.1 中分母上那两个极大值。
...
约定 $\tau_j=0$ 时 $\langle v_j,k\rangle^{\tau_j}=1$。$Z_k$ 收集那些「对频率 $k$ 完全不提供信息」的测量，$R_0$ 度量各测量对同一频率的灵敏度差异——**它就是后面样本复杂度里那个可能很大的常数**。$W$ 的对角元取负一半次幂，是因为它的用途是把 $\Psi$ 的**列范数**归一化。
...
在 $\alpha=\bigl(\max_{k\in\Gamma}\|V^\top k\|_\infty\cdot\max_{k\in\Gamma}\|V^\top k\|\bigr)^{-1/2}$ 处最优——这一点由把括号里的量看作 $\alpha^2$ 的一元函数求极值即得。对 $\Gamma=[-q,q]^d$、$V=(e_1,\dots,e_d)$、$\alpha=1/(\sqrt d\,q)$，
...
**论文明确留下的猜想。** 导数信息是否也**降低限制等距常数**（而不只是相干性），论文列为**猜想**：对 $[-\pi,\pi)^d$ 上的均匀样本，数值实验给出肯定的迹象，但没有证明。方向 $\{v_j\}$ 与权 $\alpha$ 的最优选取同样留作未来工作；从 Fourier 基推广到正交多项式基这一条，正是编号 32 所做的事。
...
标准 $\ell_1$：样本 $\Xi=\{z^{(1)},\dots,z^{(N)}\}\subset\Gamma$，$[\Phi]_{ij}=\psi_j(z^{(i)})$，$\Phi\in\mathbb R^{N\times M}$，解 $\arg\min\|c\|_1$ 使 $\Phi c=f$（或 $\|\Phi c-f\|_2\le\epsilon$）。互相干性 $\mu(\Phi)$ 与 $\mu<\frac{1}{2s-1}$ 的恢复判据如开头所述。
...
而 $W$ 为分块对角 $(W^0,W^1,\dots,W^d)$，每一块取「该块自己的正交权与采样密度之比」的平方根，也就是下面一般 Jacobi 公式在 $\alpha=\beta=0$ 处的特例；结果是 $\mathbb E_c\bigl[\frac1N\hat\Phi^\top\hat\Phi\bigr]=I$。
...
**定理没有做到什么，作者自己讲得很清楚。** 论文写道：「理想情况下我们希望证明梯度方法给出更小的参数 $\beta_L$，即 $\beta_L(\hat\Phi)\le\mu_L(\Phi)$……我们的分析没有结出这个果实。」实际证到的只有两条：(i) $\Phi$ 与 $\hat\Phi$ 的相干性界都是「一个常数的 $d$ 次幂」，且**与多项式次数无关**；(ii) 多出的因子 $C\le1+\frac{\sqrt2}{2}$ **与维数无关且很小**。此外**本文没有 $N\gtrsim s\log^k$ 形式的样本复杂度定理**，恢复保证走的是文献里 $\mu<\frac{1}{2s-1}$ 的判据。
...
论文正文与图注在样本数上有两处不一致：§5.2 中 $(d,n)=(2,20)$ 一组「恢复概率对 $s$」的固定样本数，**正文写 $N=35$，图注写 $N=50$**；$(d,n)=(10,3)$ 一组**多数处写 $N=70$，另有一处写 $N=50$**。上表按较常出现的值填写。

与理论的落差：定理 3.1 只界定了相干性参数 $\mu_L$ 与 $\beta_L$ 的上界，且明说没有证出 $\beta_L\le\mu_L$；而实验直接测的是恢复概率与逼近误差。**实验建立的是方法有效，不是定理预测了这种有效性。**
...
论文的贡献正是替换这一后处理：让**采样既不依赖输入密度、也不依赖数据驱动基**，因而可以离线完成；同时把稳定性从 $N$ 的二次改进到拟线性。达成这一点的机制还是那一个：Christoffel 加权把稳定性因子 $\kappa(N)=\max_\xi\sum_j\Phi_j^2(\xi)$ 压到它的理论下限 $N$。代价则是加权后的基不再关于 $\rho$ 正交，而是关于一个依赖多项式空间的变换测度正交——论文用位势论的极限（平衡测度）把这个依赖抹掉，**因而稳定性只在 $N\to\infty$ 的渐近意义下成立**。
...
$A=[\Phi_j(z_m)]\in\mathbb R^{M\times N}$，$W=\mathrm{diag}(w_1,\dots,w_M)$。权取归一化 Christoffel 函数值：记 $K(\xi)=\sum_{j=1}^N\Phi_j^2(\xi)$，则
...
> 下面两条密度在原文中就被标为**猜想**。Gauss 情形的极限诱导 / 平衡测度 $C(2-\|\xi\|^2)^{d/2}$ 在这一族论文中反复出现，**每一处都是猜想，从来不是定理**；有界情形的 Chebyshev 极限则有定理支撑。两者不可混为一谈。
...
（根号覆盖整个分式；$d=1$ 时它退回一维已知的 $\sqrt{(4-\xi)/\xi}$）。采样器：算出 $k$；抽参数为 $\bigl(\frac12,\frac12,\dots,\frac12,\frac d2+1\bigr)$ 的 $(d+1)$ 维 Dirichlet 向量 $y$；丢掉最后一个分量；令 $z=4ky$。注 4.1 承认：对其他无界密度**连猜想都没有**，退路是截断区域再用 Chebyshev 采样，而截断误差论文没有处理。
...
$\delta$ 为一个只依赖 $r$ 的常数。原文在范数里印的是设计矩阵 $A$ 本身，但与单位阵比较的必须是 **Gram 矩阵**：$A$ 是 $M\times N$ 的矩形矩阵，$\|A-I\|$ 无从定义。
...
**§5.1 稳定性。** 报告 $\hat A=W^{1/2}A$ 的条件数，以均值配 20% 与 80% 分位数的形式对多项式次数作图。四个 $d=2$ 的测试类型为：
...
**§5.2 精度**，用 $L$ 个参考样本上的离散 $\ell^2$ 误差 $\bigl(\frac1L\sum_l|\cdot|^2\bigr)^{1/2}$ 度量。
...
实验与理论的落差集中在一点：§5.1 测的是**有限次数**下的条件数，而定理保证只在 $N\to\infty$ 的渐近意义下成立（因为用 $\hat\rho$ 替代了 $\tilde\rho$）；情形 3、4 里的离散与无界输入更是落在定理 3.1 覆盖范围之外，或只落在猜想的密度上。**实验能说明方法在这些情形下可用，不能说明定理覆盖了这些情形。**
...
论文的替换很直接：**用支撑在 $S$ 本身上的精确诱导测度替换平衡测度，用预条件 $\ell_1$ 替换最小二乘**。诱导测度就是把 Christoffel 权反过来用——在基函数大的地方**多**采样，再用 $1/\kappa$ 加权把它抵消掉。这个「采样密度 $\propto\kappa$、权 $\propto1/\kappa$」的倒数配对，正是开头那条一致有界性原则的又一次实现，而在离散情形下它还额外买到一件事：**每个样本都落在真实数据点上**。
...
论文**没有**陈述算法 2 的样本复杂度。诸如「$M\gtrsim s\log^3s\log N$」之类的式子在编号 21、24、29 中各有其明确的假设，但它们都不是本文的结论。

### 数值实验

三个方法标签贯穿全部实验：**induced distribution**（本文方法）、**CSA**（平衡测度采样的 $\ell_1$，即[[computational-mathematics/paper-notes/stochastic-approximation/optimal-sampling-and-preconditioning|编号 24]] 的方法）、**MC**（从 $\omega$ 独立同分布采样、不加预条件的 $\ell_1$）。全部结果对 100 次独立试验取平均；精度误差在 $E=10{,}000$ 个从 $\omega$ 独立抽取的样本上以离散范数计算。
...
**§4.2 解析函数逼近。** 测试函数为 $f_1(z)=\exp\bigl(-\sum_iz_i\bigr)$；$f_2(z)=\sum_i(1-z_{i-1})^2+\sum_{i\ge2}100(z_i-z_{i-1}^2)^2$（Rosenbrock）；$f_3(z)=\sin(\sum_iz_i)$；$f_4(z)=\bigl(1+\frac{1}{2d}\sum_ic_i(1+z_i)\bigr)^{-d-1}$，$c_i=\frac{1+i}{4d}$。二维情形用图 1 的 $S$ 与 $K=20$：**$M$ 较大时诱导采样明显优于 MC 与 CSA，而 $M$ 相对较小时 MC 反而略胜**。改用 §4.1 的混合数据，以及 $d=5$、$K=7$ 的情形重复：$M$ 增大时诱导采样同样胜过 MC。

**§4.3 参数化偏微分方程。** 有界多边形区域 $D\subset\mathbb R^2$ 上的固支 **Kirchhoff 板弯曲**问题：$-M_{IJ,IJ}(u)=f$ 于 $\Gamma\times D$，$u=\partial_nu=0$ 于 $\Gamma\times\partial D$，$M_{IJ}(u)=D(z,x)\bigl((1-\nu)K_{IJ}(u)+\nu K_{LL}(u)\delta_{IJ}\bigr)$，$K_{IJ}(u)=-\partial_{IJ}u$，抗弯刚度 $D(z,x)=\frac{E(z,x)h^3}{12(1-\nu^2)}$。**随机 Young 模量**经截断 Karhunen–Loève 展开进入：$Y(z,x)=\log(E(z,x)-100)$，$Y=1+Z_1(\sqrt\pi L)^{1/2}+\sum_{i=2}^d\zeta_ig_i(x)Z_i$，协方差只沿 $x_1$ 方向取平方指数型 $K(x_1,x_1')=\exp\bigl(-(x_1-x_1')^2/L_c^2\bigr)$，特征函数是频率 $\lfloor i/2\rfloor\pi/L_p$ 的正弦与余弦函数，按 $i$ 的奇偶交替。

**实验支持的论断范围。** 本文没有定理，所以这里不存在「实验与理论的差距」问题，只有「实验支持到哪里」的问题。实验支持的是：在数据驱动、有限次数、$M$ 不太小的情形下，诱导采样通常优于平衡测度采样与朴素蒙特卡罗。它**不**支持诱导采样总是更好（§4.2 中 $M$ 小时 MC 略胜），也**不**支持任何关于所需样本数的定量结论。
...
一条贯穿的判断：**稀疏恢复的样本复杂度由行范数的均匀性决定，因此凡是改变行结构的操作（加梯度行、换基、换采样密度）都必须配一个相应的预条件。** 这条判断把这六篇与[[computational-mathematics/paper-notes/stochastic-approximation/optimal-sampling-and-preconditioning|最优采样]]一页统一起来：两者处理的是同一个量，只是一个在最小二乘框架下、一个在 ℓ1 框架下。
