...
> [!warning] 记法约定
> 定义 $K(z)=\sum_\alpha\varphi_\alpha^2(z)$，（归一化的）Christoffel 函数为 $N/K(z)$，因此下文出现的权一律写成 $1/K$ 的形式；文献中的 $\lambda_\Lambda(z)$ 即 $1/K_\Lambda(z)$。这一族论文的记法互相冲突：编号 22 与 28 用 $K$ 记 $\sum_\alpha\varphi_\alpha^2$（即 Christoffel 函数的倒数），编号 24 用 $\lambda_\Lambda$ 记 $1/\sum\varphi_i^2$（即 Christoffel 函数本身），编号 36 用 $K(\xi)$ 记 $N/\sum_j\Phi_j^2$，编号 44 与 45 用 $\kappa$、$q^2$ 记归一化的倒数。**引用任何一篇原文之前都要核对它自己的约定**，否则权会取成倒数。

### Gauss 权就是 Christoffel 函数值

这一条应当先说，因为编号 13 与 14 的构造完全建立在它之上。在一维、$\Lambda=\{0,\dots,N-1\}$ 的情形，存在一族 $N$ 点正权求积公式，对次数不超过 $2N-2$ 的多项式精确，而它的权恰恰是 Christoffel 函数值：
...
> 有界区域上诱导测度的大 $N$ 极限是（张量化的）Chebyshev 密度，这是定理。**高斯情形不是。** $D=\mathbb R^d$ 配高斯权时，$\lim_{k\to\infty}\rho(x/\sqrt k)=C(2-\|x\|_2^2)^{d/2}$ 在编号 22、28、36、45 中一律以**猜想**的身份出现，从未被证明；编号 45 直接说多元情形「更多的是未知，很多情况下我们只有猜想」。它不能当作已证结论引用。

## 6：先用实验回答「取哪些点」

编号 6 是整个采样设计纲领的经验序幕。当时的担忧很具体：**随机点会不会破坏收敛率**。这个担忧有直观基础——结构化网格（稀疏网格）是被设计出来的，随机点则可能挤成一团、留下空洞，看上去理应更差。论文不从理论上回答，而是把「哪种点更好」拆成三条通常被分开研究的判据一起测：收敛率、设计矩阵的条件数、以及函数值带数值噪声时的稳健性。测例是若干经典高维测试函数加上一个随机 ODE 模型，候选点取稀疏网格、Monte Carlo 与拟 Monte Carlo 三族。

结论是随机点并不引入低收敛率：函数有足够正则性且点够多时，Monte Carlo 与拟 Monte Carlo 仍给出高阶收敛，其中拟 Monte Carlo 是确定性的、条件数也更好，在较高维上是更好的选择；稀疏网格只在很低维（论文的说法是 $d\le2$）时收敛更好。「点够多」在这里是定性的，论文没有给出 $M$ 与 $N$ 的任何关系式——**这恰恰是编号 9、11、13、22、28、45 后来要补的洞**。它的贡献因此不是一个数学对象，而是这套比较协议本身；而后来的进展说明它的提法只对了一半：真正的问题不是收敛率，而是要让设计矩阵条件良好需要多少个样本——正是上一节那个稳定因子。此后每一篇都在回答这个加强版问题，而且用的是**构造出来的**点集而非**挑选出来的**点集：编号 9 的新配点网格、编号 11 无界区域上的随机求值、编号 13 与 21 的随机求积、编号 14 的非结构网格，以及[[computational-mathematics/paper-notes/stochastic-approximation/optimal-sampling-and-preconditioning|另一页]]上的 Christoffel 加权采样与加权近似 Fekete 点。
...
论文自己给出的定性结论就是「确定性点的表现与随机点相似」——**收益不在精度，而在去掉概率限定词**。它也坦承所取的 $d$、$q$ 与测试函数并不特殊，换成别的也差不多。

实验与定理之间有一个应当点破的距离：定理要求的前因子在 $d=2$ 时是 $4^3\cdot4=256$，在 $d=3$ 时是 $4^4\cdot9=2304$，而实验跑的是 $m=c(\#\Lambda)$ 与 $m=c(\#\Lambda)^2$。除非 $c$ 取到这个量级，实验所在的区间就根本不在定理的覆盖范围内，而条件数照样是好的。**这说明定理 3.2 是充分条件而非紧的条件**，这一点论文没有展开。
...
> [!warning] 编号 11 的记号与编号 9 冲突
> 这一篇里 $m$ 是**随机点个数**，$K$ 是**基函数个数**，而 $M$ 是被逼近函数的有效支集半径（$|f(y)|<\epsilon$ 当 $|y|>M$）。编号 9 里 $M$ 是素数、$N$ 是空间维数。读定理时务必分清。
...
并称其在实际计算中不可接受。**这是论文对数值观察到的标度的刻画，不是一条被证明的下界。**
...
**这正是共用装置一节里的行范数平方**。假设 $L>3\tau$ 与 $L>5\sqrt K$ 就是在这里用掉的：前者让引理 3.1 的衰减界生效，后者保证映射区间宽到能盖住前 $K$ 个基函数的活动范围。分析只对一维 Hermite 情形写出，论文称 Laguerre 情形可直接照搬。
...
一致的发现是：**决定成败的是丢掉坏点，而不是缩放本身**。参数化 UQ 的应用用了一个随机 ODE 模型和一个带对数正态随机输入的椭圆问题。论文另有一处报告最优缩放约为 $\alpha=0.82$。

实验支持了什么、又差在哪里，值得分开说。它确实支持了两件事：多项式混沌在无界域上的失败是真实的、可测量的；换基加映射后条件数受控，且 $L$ 是有效的旋钮。它没有支持的是收敛那一半——**$\alpha$ 的选取没有定理，只有讨论**，而两个报告出来的最优值（$2.8$ 与 $0.82$）相差三倍以上，说明它们是算例相关的观察，不是通用的取值建议。
...
设计空间的两端是清楚的：一端是从正交测度独立采样，描述简单但样本要得多；另一端是完整的张量积 Gauss 网格，离散正交性完美但基数 $\prod_i n_i$ 关于维数呈指数。编号 13 走中间那条路——**从张量 Gauss 网格里随机抽一小部分并按 Gauss 权重新加权**，保住 Gauss 求积那套优良的离散正交结构，而只付 $M$ 次昂贵求值的代价。它之所以不是一个任意的重加权方案，全靠上一节那个恒等式：**Gauss 权就是 Christoffel 函数值**，所以子采样 Gauss 网格隐含地就是 Christoffel 加权采样。

装置与姊妹篇编号 21 相同。设 $\Theta^i_{n}=\{z^i_1,\dots,z^i_n\}\subset\Gamma_i$ 是第 $i$ 维的 $n$ 点 Gauss 点集，张量集为 $\Theta_{\mathbf n}=\Theta^1_{n_1}\otimes\cdots\otimes\Theta^d_{n_d}$，$|\Theta_{\mathbf n}|=\prod_{i=1}^{d}n_i$，Gauss 权为
...
这条设计的代价藏在常数里。同一采样方案的 $\ell_1$ 版本（编号 21）的充分样本数为 $M\ge L(\mathbf n)C_1s\log^3(s)\log(N)$，而对超立方体上的均匀随机变量，Tang 与 Iaccarino 给出 $L\le C^d$，其中 $C$ 实质上是 $3$，即 $M\gtrsim3^d s$。**在张量 Gauss 网格这个受限的设计空间里，维数依赖并没有消失，它搬进了前因子**，因此这条路线并不无条件优于诱导采样的 $M\gtrsim N\log N$。

它是编号 21 的直接前身——同一批作者、同一个「随机子采样 Gauss 求积」的装置，只是用最小二乘而非 $\ell_1$ 极小化。它的 Gauss 权就是 Christoffel 函数值，因此向前接上编号 22 的显式 Christoffel 加权框架与编号 45 的诱导采样理论。
...
> 所印的两式 $\rho(z)=\exp(\|z\|^r)$ 与 $Q(z)\triangleq-\log\rho=\|z\|^r$ 不能同时成立：由前者应得 $-\log\rho=-\|z\|^r$。其中有一个符号误植，按该领域的标准写法，正确的取法是 $\rho=\exp(-\|z\|^r)$ 配 $Q=\|z\|^r$，即权本身应带负号，否则它在无穷远处不衰减，加权平衡测度也不会有紧支集。
...
这里最该指出的一点是实验规则的选取：$M=2.5N$ 与 $M=1.5N\log N$ 分别是**线性**与**对数线性**，也就是说，**该文的实验刻意跑在它自己的定理 1、5、6 所要求的二次区间之下**。这不是疏忽，正是那张表最后一行的实证形态：实践在线性区间里就能工作，而覆盖该区间的理论当时还没有。
...
| 13   | 张量 Gauss 网格的随机子集              | 同一设计下 $M\gtrsim L(\mathbf n)s\log^3 s\log N$，$L\le3^d$             | 与编号 21 共用设计    |
