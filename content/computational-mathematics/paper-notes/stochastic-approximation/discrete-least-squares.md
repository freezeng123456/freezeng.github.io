...
> [!note] 覆盖进度
> 编号 9、11、14 的定理与样本复杂度陈述已按原文核对，编号 6 只到摘要与元数据层面。编号 13 无法核实：出版社封锁正文，researchr 明确记录「摘要缺失」，因此其构造是按设计完全相同的姊妹篇（编号 21）反推的，而「样本数线性增长」这一说法只有第三方文献（Seshadri-Narayan-Sarkar）的表述支持，并非出自该文本身；其常数与概率界本站不报告。
