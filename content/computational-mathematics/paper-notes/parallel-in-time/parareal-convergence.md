...
这两组数字分别出自 Gander、Wu 与 Zhou 的 _Acta Numerica_ 综述（编号 85）与 Gander–Vandewalle（2007，Table 5.1），不是本页四篇论文自己的实验，此处只用来给下面各篇的常数一个可比的量级。
...
门槛在 $z_{\max}=\Delta T\lambda_{\max}$ 上的**对数平方**增长，是论文所说「$J_{\rm cri}$ 依赖 $\Delta T$、$\Delta t$ 与 $\lambda_{\max}$」这句话的精确形式，也是全文的核心技术内容。
...
**（三阶 DIRK 的特殊性。）** 对三阶两级 DIRK 细传播子，$J_{\rm cri}=4$，**与 $\Delta T$、$\Delta t$、$\lambda_{\max}$ 全部无关**。这是三个结论里唯一一个不带参数依赖的，也是这条线索里最实用的一条：一个固定的、可以直接写进代码的粗细比。对梯形规则与四阶 Gauss RK，论文另给出计算 $J_{\rm cri}$ 的简明闭式公式。

### 数值实验

数值算例分两类——含分数阶 PDE 的问题与不确定性量化应用——用来支持理论预测。这两类不是随手选的：
...
这条文献里围绕本文流传的收缩常数都聚在 $0.3$ 附近：Mathew–Sarkis–Schaerer 的 parareal-Euler 为 $\approx0.298$（$J\ge2$），作者自己的综述把本文的结论四舍五入为 $\approx0.3$，并记录两个具体门槛——两级二阶 SDIRK 为 $J_{\min}=2$（Wu, IMA J. Numer. Anal. 2015），两级三阶 SDIRK 为 $J_{\min}=4$（本文），后者正是上面的 $J_{\rm cri}=4$。
...
该文在**常步长**下分析含分数阶 Laplacian $(-\Delta)^{\alpha}$ 的时间依赖 PDE 上的 parareal。空间半离散给出稠密、对称正定、$\lambda_{\max}$ 极大的 $A$（一维情形有 Toeplitz 结构），这打破了 parareal 的成本模型：一次隐式求解本身就变得昂贵，而粗传播子每轮都要在 $N_t$ 个粗区间上顺序地做这件事，直接落在关键路径上，于是即使三四轮就收敛，运行时间也可能被那条顺序链条吃掉。同时，极大的 $\lambda_{\max}$ 正落在编号 12 的门槛按 $O(\log^2(\Delta T\lambda_{\max}))$ 增长的区域，两个困难叠加：粗传播越贵，越希望取大的 $\Delta T$，而大的 $\Delta T$ 又抬高 $z_{\max}$ 与所需的 $J$。题名中的「快速」因此指加速 parareal **迭代本身**，而不只是给出收缩因子的界。

它的位置由两条对照定出。编号 12 是直接前身：它建立了对称正定 $A$ 上的 $J_{\rm cri}$ 框架，并已把分数阶 PDE 作为动机算例。编号 30 是另一种分数阶的对应物——编号 20 处理**空间**分数阶（空间非局部、时间局部，parareal 的结构得以保留），编号 30 处理**时间**分数阶（时间非局部，破坏子区间之间的独立性）。至于[[computational-mathematics/paper-notes/parallel-in-time/diagonalization-technique|编号 31]]，它同样面对分数阶 Laplacian，却完全放弃迭代，转向时间周期情形的直接对角化。
...
时间分数阶微分方程，分数阶算子具有历史效应。目标是设计一个**代价在各进程间均衡**且收敛速率稳健的 parareal 算法。论文给出的路线是采用两个「近期发展出来的时间分数阶算子的局部时间积分器」，两者都通过引入辅助变量来局部化分数阶算子。
...
**为什么必须分开。** 解变量与辅助变量在动力学上的角色不同：辅助变量是为逼近记忆核而引入的，其模态的时间尺度分布很宽（这正是核压缩的目的），把它们和解变量一起用同一个粗传播子校正，等于用同一个 $\Delta T$ 去处理跨若干数量级的时间尺度。分开校正使两类变量各自获得合适的处理。（这是对分开校正的机理解读，不是论文自己给出的论证。）

### 定理

论文的结论是：所提出的 parareal 算法**具有稳健的收敛速率**。在这一系列工作中，「稳健」一贯指与空间算子的特征值、粗细比 $J$ 以及粗区间数 $N_t$ 无关。
...
这是一篇纯分析工作，研究**均匀粗网格**配**任意分布的非均匀细网格**时的 parareal 收敛行为，迭代式本身不变。「任意分布」是实质所在：粗区间内的细网格既不假设均匀，也不假设分级，甚至不假设各粗区间之间相同。自适应时间步、初值不光滑或不相容时 $t=0$ 附近的分级网格、快速瞬态附近的局部细化都会产生这样的网格，因此把理论限制在均匀细网格上等于排除了几乎全部自适应实践。

它之所以不是记号上的推广，在于全部经典线性收敛结果——Gander 与 Vandewalle 的界、$0.3$ 附近的常数、编号 12 的临界粗细比公式、编号 39 的 MGRIT 因子——都依赖「粗区间上的细传播是单个标量稳定函数在单个自变量上的 $J$ 次幂 $R_f^{J}(z/J)$」这一恒等式，而它要求区间内的 $J$ 个细步等长。细网格非均匀时得到的是**互不相同因子的乘积**
...
于是两件事失效：$\varrho_l(J,z)$ 不再有定义，因为没有单一的 $J$；而若细网格在各粗区间之间不同，则 $M_f(z)$ 的次对角元素逐行变化，$M_f(z)$ **不再是 Toeplitz** 矩阵，本页开头那个把误差矩阵写成「标量因子乘固定 Toeplitz 矩阵」的分解、以及作用在 $\widetilde M$ 上的 Gander–Vandewalle 引理，都失去了作用对象。在这一设定下恢复稳健的、与网格无关的收缩因子因此是全新的分析。

它与编号 12 的关系正在这里：编号 12 固定 $\mathcal G$ 为后向 Euler、允许三种 $\mathcal F$，在**均匀细网格**上找临界比 $J_{\rm cri}$，结论依赖标量 $J$；编号 77 保留均匀粗网格但完全放开细网格，那里根本没有一个标量 $J$ 可以依赖。[[computational-mathematics/paper-notes/parallel-in-time/diagonalization-technique|编号 39]] 通过 MGRIT 收敛因子中的 $|R_f^J(z/J)|$ 继承了同一条均匀细网格限制，因此编号 77 的技术是通向非均匀网格 MGRIT 理论的自然路径；编号 20 与编号 30 都在常步长下做分析，编号 77 正是把这条长期假设从这一组工作中移除的那一篇。
