...
方法是把编号 8 的结果加以推广而得。本文特别标出的贡献是：「在我们的数值格式中，**可以自由选择相关联的正向随机微分方程**，而恰当的选择能显著降低计算复杂度。」

这条自由的来源上一节已经说清：CSTV 的 PDE 不依赖 $\mu,\sigma$，而编号 8 的定理 2 说一步导数逼近只看左端点系数。两者叠起来，就允许挑一个方便的辅助扩散。具体地说，若把系数取成常数，则每个网格点上可以复用同一组 Gauss-Hermite 节点，省掉逐点重算。数值例子里包括 HJB 方程。

### 与其他论文的关系

编号 16 与 19 是近乎孪生的两篇：**16 从 PDE 出发（全非线性抛物方程 → 2FBSDE → 格式），19 从 2FBSDE 出发并接上随机控制应用。** 编号 16、19、23、25 在各自的参考文献里互相引作预印本或投稿中的稿件，是同一批同时展开的工作。编号 35 后来把这里的多步模板换成显式延迟校正，处理同一个 2FBSDE 类。
...
> [!warning] 这篇同样没有收敛定理
> 论文**没有**证明带显式常数的收敛定理；高阶性由上面的截断误差估计加数值支撑。
...
控制由 $\alpha^n=-\frac{\beta}{2q\sigma}Z^n$ 恢复，参数 $\mu=0.1$、$r=0.03$、$\sigma=0.5$、$c=0.1$。论文的结论是「该方法对 2FBSDE 的解与最优控制 $\alpha$ 都具有高阶精度」。
...
**这些实验建立了什么，又差在哪里。** 建立的是：二阶设定下四个分量同时达到 $k$ 阶，这是本文的核心主张，而表 5.4 直接支持它；效率对比也表明高阶在实测中确实划算。差在三处。第一，例子仍是低维光滑问题，条件期望用张量 Gauss-Hermite；论文自己在引言里把「高阶只在低维、高维只有低阶」当作研究缺口，而这一篇解决的是前半句，没有解决后半句。第二，四精度算术是一项实质的实现选择，说明这一族在双精度下能否稳定跑到 $k=6$ 仍是问题（编号 8 在双精度下的 $k=6$ 就被舍入污染了）。第三，控制例子的高阶只有文字论断，没有给出对应的阶。
...
**本文没有自证的收敛定理。** 它把编号 8 的 $k$ 阶结论直接搬过来，并引用文献中已知的稀疏网格求积误差结果。它的实质贡献是复杂度层面的：运行时间随维数「按某种多项式（而非指数）水平」增长——**这是一条数值观察（图 2），不是定理。**
...
**这些实验建立了什么，又差在哪里。** 建立的是：稀疏网格把同一套时间格式推到六维仍保持高阶，且时间增长曲线不再是指数——这正是本文要证明的那一件事。差在三处。第一，**六维是这条路线的实测天花板，不是它的下限**；后来的深度学习几篇（编号 86、93、96、97、100）跑到上千维，靠的是彻底放弃网格。第二，例 1 的三步格式在 $E_Y$ 上只观察到 2.632，明显低于名义的 3，而 $E_Z$ 是 2.955——**误差表里 $Y$ 与 $Z$ 的阶不同步，论文没有解释这一处。** 第三，例 1 依赖周期性把定义域压到有界方体上，这是稀疏网格能用的前提；一般无界问题上如何截断，实验没有覆盖。
...
**例 1。** $J(u)=\frac12\int_0^T\mathbb E[(x_t-x^\ast(t))^2]\mathrm dt+\frac12\int_0^Tu^2(t)\mathrm dt$，$K=U$，状态 $\mathrm dx_t=u(t)x_t\mathrm dt+\sigma x_t\mathrm dW_t$。设定 $x_0=1$、$T=1$、$\sigma=0.1$、$M=10^5$ 个样本、容差 $\epsilon_0=10^{-5}$，观察到一阶收敛。另一组测试取 $\sigma=0.1$、$M=10^5$、$\epsilon_0=10^{-5}$、$N=40,50,\dots,100$，结论是「数值解与精确解吻合很好，观察到一阶收敛率」。
...
**这些实验建立了什么，又差在哪里。** 建立的是：定理 2 预言的一阶在四个例子上都被观察到，而反馈控制的对比表给出了一条独立于阶数的、关于**控制集大小**的结论。差在两处。第一，一阶是这一篇的设计目标而非局限的证据——它有意只用 Euler 格式，配一阶分析；真正的问题是能否更高阶，那是编号 41 的题目。第二，全部例子都是一维的，而论文对多维只声明「框架容易推广」，没有给出多维实验。
...
论文先把随机最优控制问题转化为「等价的 FBSDE 随机最优性系统」，再为所得系统设计一个高效的**二阶 FBSDE 求解器**与一个**拟 Newton 型优化求解器**。它特别标出的一点是：**即使状态方程用 Euler 格式逼近，方法仍具有二阶收敛率**。论文没有给出所用二阶格式的具体形式，也没有给出拟 Newton 更新的具体形式。

两个部件各自站在不同的传统上：高阶 FBSDE 求解器来自多步／延迟校正工具箱（论文引用编号 8、19、23、25、26 与 Zhao-Zhang-Ju、Zhao-Chen-Peng），优化器来自非线性规划文献（Powell-Yuan 的信赖域、戴彧虹与袁亚湘的共轭梯度、何炳生的投影收缩方法）。
...
**论文没有给出 PF-SGD 的收敛定理。** 它转而引用外部分析（对上述倒向格式及其推广的数值分析见其参考文献，即 Bao-Cao-Webster 型与赵卫东学派的分析），以及标准的随机梯度下降收敛文献。**因此这是一篇算法／计算的论文，不是误差分析的论文。** 唯一明确陈述的理论部件是附录中由随机最大值原理与 Gâteaux 导数推出的梯度表示。
...
报告的结论是该方法在稳健性、复杂度与计算时间三方面都优于已有方法，且两个理论区间——固定 $T$ 时对 $h$ 稳健的超线性收敛，与固定 $J$ 时的 $O(h)$ 线性收敛——都被观察到。
...
| 编号 | 处理的对象               | 相对其他篇的位置             |
| ---- | ------------------------ | ---------------------------- |
| 16   | 全非线性抛物方程         | 二阶设定的起点，PDE 优先框架 |
| 19   | 二阶 FBSDE 与随机控制    | 把二阶设定接到控制问题       |
| 25   | 多维条件期望的求值代价   | 空间表示方向的改进           |
| 26   | 带约束的随机最优控制     | 外层迭代（梯度投影），一阶   |
| 41   | 随机最优控制的高精度格式 | 内层求解（高阶 FBSDE），二阶 |
| 50   | 数据驱动反馈控制         | 部分观测，滤波与控制耦合     |
...
- 相关来源：P. Cheridito, H. M. Soner, N. Touzi, and N. Victoir, [_Second-order backward stochastic differential equations and fully nonlinear parabolic PDEs_](https://doi.org/10.1002/cpa.20168), Comm. Pure Appl. Math. 60(7) (2007), pp. 1081-1110（二阶设定的定义、PDE 对应与「正向扩散可自由选择」的出处）。
