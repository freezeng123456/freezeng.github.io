...
窗口 $1\le k\le6$ 不是含糊的经验观察，而是一次可以复算的零稳定性检验。把同一组权用到确定性常微分方程 $\mathrm dY/\mathrm dt=f(t,Y)$ 上得
...
**根模在 $k=6$ 与 $k=7$ 之间越过 $1$，因此格式对 $k\ge7$ 不稳定**，这就是只列到 $k=6$ 的原因。编号 8 缺的不是这条计算，而是**随机**情形下的稳定性理论；那要等到编号 47（见[[computational-mathematics/paper-notes/fbsde-and-control/stability-theory-for-fbsdes|稳定性理论一页]]），那里的根条件正是「稳定性」这一半。
