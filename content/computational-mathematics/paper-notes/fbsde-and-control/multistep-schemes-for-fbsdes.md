...
> [!warning] 这篇没有收敛定理
> 论文**没有**给出带显式常数的严格收敛定理。高阶性由上面的截断误差估计加数值证据支撑；谈到格式 4 时的原话只是「我们可以期望格式 4 是一个高阶数值格式……我们将用数值验证这一点」。这一族的严格稳定性与收敛性分析要到编号 47 才出现，见[[computational-mathematics/paper-notes/fbsde-and-control/stability-theory-for-fbsdes|稳定性理论一页]]。
...
受跳扩散过程局部性质的启发，正向的带跳随机微分方程只用 Euler 方法求解，这大幅降低整体计算复杂度，而带跳倒向方程中关心的量仍保持高阶收敛率。每个时间步中，计算过程只涉及**一次跳**，这再次大幅降低复杂度。借助广义 Feynman-Kac 公式，方法容易应用于偏积分微分方程以及某些非局部 PDE 模型。
...
- 对 FBSDE，论文**声称** $K$ 阶并用数值验证，但没有给出带显式常数的 FBSDE 收敛定理。
...
**测试 1，耦合系统 (5.3)。** $\mathrm dX_t=\frac{1}{1+e^{t+X_t}}\cdot\frac{1}{1+Y_t}\mathrm dt+Y_t\,\mathrm dW_t$，倒向方程与终值同上，$x=0$。$K=1$ 的收敛率为 $0.937/1.008$；更高的 $K$ 沿同样的 $K$ 阶模式（$K=2$ 的误差自 $3.666\times10^{-4}$ 起）。
...
**这些实验建立了什么，又差在哪里。** 建立的是：延迟校正在一维光滑问题上确实按 $K$ 阶收敛，包括耦合情形，而且**做到了 $K=12$——远超多步族的 $k\le6$ 壁垒**，这是这条路线相对多步法最实在的证据。差在两处。第一，这一切都是数值观察，论文没有给出 FBSDE 的收敛定理，因此「为什么没有壁垒」在这一篇里仍是空白（编号 47 的框架给出了讨论它的语言）。第二，论文自己给出的效率论断只有 $N=4,K=2$ 对 $N=12,K=1$ 这一处，不足以支持任何跨 $K$ 的一般效率比较。
...
论文提出一族**显式 $\theta$ 格式**：先证明一个严格的**稳定性结果**，再在此基础上给出**尖锐的误差估计**，表明该 $\theta$ 格式具有**二阶收敛率**。论文自述这似乎是首次尝试为均场倒向随机微分方程设计高阶数值格式——此前的均场格式都是一阶。
...
论文自述为「我们关于正倒向随机微分方程延迟校正方法系列论文的第二部分」，直接续编号 23，提出一类用于 2FBSDE 的**显式**延迟校正格式——「显式」是与编号 23 的关键区别。关键特征是用**简单的 Euler 格式作为初始化**，随后通过一个简单的延迟校正迭代格式得到精度很高的近似解，而每次迭代的**计算复杂度始终与 Euler 求解器相当**。论文没有给出所达阶数、假设或常数。
...
格式是**显式多步**的、易于实现、具有高阶收敛率，并给出了所提多步格式的**严格误差估计**。论文没有给出所达阶数的具体数值，也没有说明 $\mathbb E'$ 这一层对分布的期望究竟如何离散——而这是任何均场格式最关键的一环。

### 与其他论文的关系

编号 33 的直接续篇（并引用它）：33 对均场 BSDE 用 $\theta$ 格式达到二阶，61 对均场 FBSDE 用多步格式声称高阶。方法论上的父辈是编号 8。第二作者杨杰与编号 35、47 相同，编号 47 的统一框架也被本文引用。
...
论文先对一般类型的 FBSDE 多步格式做全面分析，据此给出**关于系数的新充分条件**使相应格式稳定且具有一定的相容阶，再据此提出**构造高阶强稳定保持多步格式的实用方法**。实用产出是一张系数表：附录 A 的表 7 给出均匀时间划分下**阶数至多 5** 的最优 SSPM 格式的系数。

强稳定保持的问题是：对哪些多步系数，格式可以改写成若干类后向 Euler 步的**凸组合**，从而基础步的任何单调性或收缩性被继承，并且步长系数尽可能大。这与 Lenferink（_Numer. Math._ 55 (1989) 213-223；_Math. Comp._ 56 (1991) 177-199）以及 Spiteri 与 Ruuth（_SIAM J. Numer. Anal._ 40 (2002) 469-491）的强稳定保持线性多步理论完全对应，那三篇都在本文参考文献中。
...
| 编号 | 加阶手段           | 问题类型       | 稳定性依据         |
| ---- | ------------------ | -------------- | ------------------ |
| 8    | 微分成参考 ODE     | 耦合 FBSDE     | 根条件窗口 $k\le6$ |
| 18   | 多步 + 跳处理      | 带跳 FBSDE     | —                  |
| 23   | 延迟校正           | FBSDE          | 数值断言，无壁垒   |
| 33   | 显式 $\theta$ 格式 | 均场 BSDE      | 论文自证的稳定性   |
| 35   | 显式延迟校正       | 二阶 FBSDE     | —                  |
| 61   | 显式多步           | 均场 FBSDE     | —                  |
...
- 相关来源：W. Zhao, G. Zhang, and L. Ju, [_A stable multistep scheme for solving backward stochastic differential equations_](https://doi.org/10.1137/09076979X), SIAM J. Numer. Anal. 48(4) (2010), pp. 1369-1394；W. Zhao, L. Chen, and S. Peng, [_A new kind of accurate numerical method for backward stochastic differential equations_](https://doi.org/10.1137/05063341X), SIAM J. Sci. Comput. 28(4) (2006), pp. 1563-1581；L. Teng, A. Lapitckii, and M. Günther, _A multi-step scheme based on cubic spline for solving backward stochastic differential equations_, [arXiv:1809.00324](https://arxiv.org/abs/1809.00324)（Zhao-Zhang-Ju 参考方程与稳定窗口的来源）。
