...
- **93（深度随机差分方法）** 面向高维准线性抛物方程。名称中的「随机差分」指用随机差分替代导数，因此可以进一步降低对自动微分的依赖。
- **96（极高维准线性方程与随机最优控制的鞅型深度学习）** 是投向 _SIAM Review_ 的综合性工作，把鞅型框架系统化并推向极高维度。
- **100（无导数局部化随机方法）** 去掉对导数的依赖，面向极高维半线性抛物方程。「局部化」指把全局问题分解为局部子问题以控制方差。
...
> [!note] 覆盖进度
> 编号 86 的内容已逐式核对（含出版版本）。编号 93、96、97、100、108 的精读仍在补充：本页只给出可从题名、摘要与编号 86 的框架关系确认的定位，未展开各自的公式与定理。
