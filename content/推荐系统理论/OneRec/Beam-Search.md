...
`pam_beam_width` 是策略/实验传给 Decoder 的主宽度。新 AnyRecall 重构路径中：

- 缺省回退值：180；
- 硬约束：不得超过模型导出的最大宽度 180。

旧 retrieval engine 存在默认 200。两者属于不同调用链和模型约束，不能因为旧配置为 200 就把新路径也改为 200。
...
| 场景                |       观察到的值 | 正确解释                                            |
| ------------------- | ---------------: | --------------------------------------------------- |
| 新生成式重构路径    | fallback/max 180 | 请求 beam 的默认与模型上限                          |
| 旧 retrieval engine |      default 200 | legacy 路径默认，不能跨路径套用                     |
| 某版本样本/实验配置 |       90、200 等 | 必须按参数 ID 的 callsite 区分 beam、top-k 或图档位 |
