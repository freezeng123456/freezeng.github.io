...
- **95 (UQ-SONet)** keeps the conditional variational-autoencoder bound but replaces the confidence gate with a permutation-invariant set encoder built on attention pooling, so sensor count and placement may vary from sample to sample; the reference distribution for stochastic operators comes from exact Gaussian-process conditionals.
- **98 (LVM-GP)** follows the confidence-aware encoder idea while replacing the noise term with a **Gaussian process prior**. The reason is a heuristic double-integral argument: for $\lambda u_{xx}=f$ the solution is a double integral of $f$, so the variance of $u$ is a global aggregation of the variance of $f$ over the region, which pointwise independent latent noise cannot represent. Physics enters as a hard structural identity because $\sigma_u$ is not differentiated in $x$.
- **107 (transition-density-based operator learning)** generalises "one equation, one flow" into learning the **solution operator** over initial conditions: a conditional flow whose base distribution is a linearised stochastic differential equation, collapsing to a Dirac measure as $t\to0$ and thereby absorbing the short-time singularity; this contrasts with the single-equation setting of papers 64, 72 and 87.
...
- **94 (deep learning for the semi-classical limit)** uses a Heller Gaussian wave-packet ansatz to eliminate the spatial variable outright, reducing the PDE to a system of ordinary differential equations for four scalars, with the high-frequency structure put analytically into the reconstruction formula. The price is that those scalars sit inside $\exp[\mathrm i(\cdot)/\varepsilon]$, so the ODE error is amplified by $1/\varepsilon$ — which is exactly why the paper uses a multi-scale network rather than a plain PINN.
- **101 (overcoming spectral bias via cross-attention)** attacks the same problem from another angle, keeping a fixed bank of multiscale random Fourier features and learning input-dependent attention weights over it instead of rebuilding the network around captured modes. **Its more important contribution is a counterintuitive observation: under physics-informed training the direction of spectral bias reverses**, because a Deep Ritz energy term weights mode $k$ by $k^2$ and a squared PDE residual by $k^4$, so the differential operator amplifies high frequencies and what ends up under-resolved is the low-frequency part.
...
Paper 101 adds a stronger qualification: **"biased toward low frequencies" is not an intrinsic property of the network but a property of the loss.** Under a regression loss the network favours low frequencies; under a $k^4$-weighted residual loss it favours high ones. So what really has to be avoided is not a bias in one particular direction but assuming a direction without measuring it.

### Every adaptation that works rests on a computable indicator

This runs across all five close-reading pages: paper 70 uses the failure probability, paper 80 the statistical variance of the variational loss, papers 81 and 101 the discrete Fourier spectrum of the current solution, paper 90 the subdomain residual, paper 103 the physics residual $\mathcal R_t$, and paper 89 the free-energy lower bound and relative effective sample size. All of these can be computed on the **current approximation** with no reference solution — which is exactly why they are usable on real problems.
