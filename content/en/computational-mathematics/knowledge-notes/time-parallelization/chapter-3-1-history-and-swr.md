...
The optimized parameter scales as $p^*=\widetilde p^*/\nu$. Define the single-frequency factor
...
**Theorem 3.2.** For $0<\alpha<\beta<1$, if
...
3. Return to the red subdomains and advance to $T_3$.
4. Continue alternating colors until the exact region reaches $T_4$.
...
Unmapped tent pitching (UTP) executes red–black SWR directly and can also be viewed as restricted additive Schwarz on the all-at-once system. It retains the original coordinates and avoids mapping-induced order reduction. Higher-dimensional implementations can reuse established RAS infrastructure.
